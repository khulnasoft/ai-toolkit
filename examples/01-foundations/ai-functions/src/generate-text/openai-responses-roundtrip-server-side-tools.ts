...
run(async () => {
  const { content } = await generateText({
    ...
    prompt: 'Search for San Francisco tech news, then check server-side tool status.',
  });

  console.log('\n=== Results ===');
  for (const part of content) {
    if (part.type === 'tool-call') {
      console.log(`Tool Call: ${part.toolName} (providerExecuted: ${part.providerExecuted})`);
    } else if (part.type === 'tool-result') {
      console.log(`Tool Result: ${part.toolName} (providerExecuted: ${part.providerExecuted})`);
