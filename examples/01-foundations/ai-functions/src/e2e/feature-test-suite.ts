...
const shouldRunTests = (
  ...
  return capabilities ? requiredCapabilities.every(cap => capabilities.includes(cap)) : false;
};
  ...
  return () => {
    ...
    describe(`${name} Feature Test Suite`, () => {
      ...
      describe.each(createModelObjects(models.languageModels))(
        ...
        ({ model, capabilities }) => {
          describeIfCapability(capabilities, ['textCompletion'], 'Basic Text Generation', () => {
          ...
          describeIfCapability(capabilities, ['objectGeneration'], 'Object Generation', () => {
            ...
            it('should generate nested objects', async () => {
              const result = await generateObject({
                ...
                prompt: 'Generate a user profile with contact details and preferences.',
              });
            ...
            it('should generate arrays of objects', async () => {
              ...
              expect(result.object.posts[0].comments.length).toBeGreaterThanOrEqual(1);
            });
            ...
            it('should handle cross-referenced schemas', async () => {
              ...
              const result = await generateObject({
                ...
                prompt: 'Generate a skincare routine with morning and evening products.',
              });
              ...
              expect(result.object.morning_routine[0].morning_specific_instructions).toBeTruthy();
              expect(result.object.evening_routine[0].evening_specific_instructions).toBeTruthy();
            });
            ...
            it('should stream complex nested objects', async () => {
              const result = streamObject({
                ...
                prompt: 'Generate a book outline with chapters, sections, and subsections.',
              });
            ...
            describe('Schema and Prompt Variations', () => {
              it('should generate with field descriptions', async () => {
                const result = await generateObject({
                  ...
                  schema: z.object({
                    ...
                    summary: z.string().describe('A 2-3 sentence overview of the main points'),
                    readingTime: z.number().describe('Estimated reading time in minutes'),
                    targetAudience: z.array(z.string()).describe('The intended reader groups'),
                  }),
              ...
              it('should generate complex objects with both descriptions and system context', async () => {
                const ProductSchema = z.object({
                  name: z.string().describe('Product name, should be unique and memorable'),
                  price: z.number().describe('Price in USD, should be competitive for market'),
                  features: z.array(z.string()).describe('Key selling points, 3-5 items'),
                  marketingPlan: z
                    .object({
                      targetMarket: z.string().describe('Primary customer demographic'),
          ...
          describeIfCapability(capabilities, ['searchGrounding'], 'Search Grounding', () => {
      ...
      if (models.embeddingModels && models.embeddingModels.length > 0) {
        describe.each(createModelObjects(models.embeddingModels))(
          ...
          ({ model, capabilities }) => {
            describeIfCapability(capabilities, ['embedding'], 'Embedding Generation', () => {
              ...
              it('should generate multiple embeddings', async () => {
                const result = await embedMany({
                  model,
                  values: ['First test sentence.', 'Second test sentence.', 'Third test sentence.'],
                });
      ...
      if (models.imageModels && models.imageModels.length > 0) {
        describe.each(createModelObjects(models.imageModels))(
          'Image Model: $modelId',
          ({ model, capabilities }) => {
            describeIfCapability(capabilities, ['imageGeneration'], 'Image Generation', () => {
