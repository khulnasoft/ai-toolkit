---
title: Stream Object
description: Learn how to stream object using the AI TOOLKIT and Next.js
...
```ts title='app/api/use-object/schema.ts'
import { z } from 'zod';
...
On the server, we use [`streamObject`](/docs/reference/ai-toolkit-core/stream-object) to stream the object generation process.

```typescript title='app/api/use-object/route.ts'
...
```tsx title='app/page.tsx' {7,16,21,24}
'use client';
