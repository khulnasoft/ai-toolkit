...
description: Build a chatbot that renders and memoizes Markdown responses with Next.js and the AI TOOLKIT.
tags: ['next', 'streaming', 'chatbot', 'markdown']
---


When building a chatbot with Next.js and the AI TOOLKIT, you'll likely want to render the model's responses in Markdown format using a library like `react-markdown`. However, this can have negative performance implications as the Markdown is re-rendered on each new token received from the streaming response.
...
```tsx title='components/memoized-markdown.tsx'
...
import { Chat, useChat } from "@ai-toolkit/react";
