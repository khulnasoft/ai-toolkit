...
description: Learn how to build an agent that has access to web with the AI TOOLKIT and Node
tags: ['node', 'tool use', 'agent', 'web']
---

# Web Search Agent

There are two approaches you can take to building a web search agent with the AI TOOLKIT:
...
When using tools for web search, you have two options: use ready-made tools that integrate directly with the AI TOOLKIT, or build custom tools tailored to your specific needs.
...
If you prefer a ready-to-use web search tool without building one from scratch, there are several options that integrate directly with the AI TOOLKIT.
...
```bash
pnpm install @exalabs/ai-toolkit
```
...
For more configuration options and customization, see the [Exa AI TOOLKIT documentation](https://docs.exa.ai/reference/vercel).

#### Parallel Web

<Note>Get your API key from the [Parallel Platform](https://parallel.ai).</Note>

First, install the Parallel Web AI TOOLKIT tools:

```bash
pnpm install @parallel-web/ai-toolkit-tools
```
...
```bash
pnpm install @perplexity-ai/ai-toolkit
```
...
```bash
pnpm install @tavily/ai-toolkit
```
...
import { generateText, stepCountIs } from 'ai-toolkit';
__PROVIDER_IMPORT__;
import { tavilySearch, tavilyExtract } from '@tavily/ai-toolkit';

const { text } = await generateText({
  model: __MODEL__,
  prompt: 'When was the latest update to the AI TOOLKIT?',
  tools: {
...
For more customization options over your agent's web-access functionality, visit the [Tavily AI TOOLKIT Documentation](https://docs.tavily.com/documentation/integrations/vercel).
