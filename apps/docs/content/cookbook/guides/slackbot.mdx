...
description: Learn how to use the AI TOOLKIT to build an AI Agent in Slack.
tags: ['agents', 'chatbot']
---
...
```typescript title="lib/handle-app-mention.ts"
...
```typescript title="lib/handle-messages.ts"
...
```typescript title="lib/generate-response.ts"
...
1. Uses the AI TOOLKIT's `generateText` function to call Anthropic's `claude-sonnet-4.5` model
