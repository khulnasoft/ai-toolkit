...
The [AI TOOLKIT](/) is a powerful TypeScript toolkit for building AI application with large language models (LLMs) like Llama 3.1 alongside popular frameworks like React, Next.js, Vue, Svelte, Node.js, and more
...
At the center of the AI TOOLKIT is [AI TOOLKIT Core](/docs/ai-toolkit-core/overview), which provides a unified API to call any LLM. The code snippet below is all you need to call Llama 3.1 (using [DeepInfra](https://deepinfra.com)) with the AI TOOLKIT:
...
<Note>
  Llama 3.1 is available to use with many AI TOOLKIT providers including
  [DeepInfra](/providers/ai-toolkit-providers/deepinfra), [Amazon
  Bedrock](/providers/ai-toolkit-providers/amazon-bedrock),
  [Baseten](/providers/ai-toolkit-providers/baseten)
  [Fireworks](/providers/ai-toolkit-providers/fireworks), and more.
</Note>

AI TOOLKIT Core abstracts away the differences between model providers, allowing you to focus on building great applications. Prefer to use [Amazon Bedrock](/providers/ai-toolkit-providers/amazon-bedrock)? The unified interface also means that you can easily switch between models by changing just two lines of code.
...
To stream the model's response as it's being generated, update your code snippet to use the [`streamText`](/docs/reference/ai-toolkit-core/stream-text) function.
...
While text generation can be useful, you might want to generate structured JSON data. For example, you might want to extract information from text, classify data, or generate synthetic data. AI TOOLKIT Core provides two functions ([`generateObject`](/docs/reference/ai-toolkit-core/generate-object) and [`streamObject`](/docs/reference/ai-toolkit-core/stream-object)) to generate structured data, allowing you to constrain model outputs to a specific schema.
...
The AI TOOLKIT supports tool usage across several of its functions, including [`generateText`](/docs/reference/ai-toolkit-core/generate-text) and [`streamUI`](/docs/reference/ai-toolkit-rsc/stream-ui). By passing one or more tools to the `tools` parameter, you can extend the capabilities of LLMs, allowing them to perform discrete tasks and interact with external systems.

Here's an example of how you can use a tool with the AI TOOLKIT and Llama 3.1:
...
### Implementing Agents with the AI TOOLKIT

The AI TOOLKIT supports agent implementation through the `maxSteps` parameter. This allows the model to make multiple decisions and tool calls in a single interaction.
...
AI TOOLKIT Core can be paired with [AI TOOLKIT UI](/docs/ai-toolkit-ui/overview), another powerful component of the AI TOOLKIT, to streamline the process of building chat, completion, and assistant interfaces with popular frameworks like Next.js, Nuxt, and SvelteKit.
...
With four main hooks — [`useChat`](/docs/reference/ai-toolkit-ui/use-chat), [`useCompletion`](/docs/reference/ai-toolkit-ui/use-completion), and [`useObject`](/docs/reference/ai-toolkit-ui/use-object) — you can incorporate real-time chat capabilities, text completions, streamed JSON, and interactive assistant features into your app.

Let's explore building a chatbot with [Next.js](https://nextjs.org), the AI TOOLKIT, and Llama 3.1 (via [DeepInfra](https://deepinfra.com)):
...
The AI TOOLKIT's React Server Components (RSC) API enables you to create rich, interactive interfaces that go beyond simple text generation. With the [`streamUI`](/docs/reference/ai-toolkit-rsc/stream-ui) function, you can dynamically stream React components from the server to the client.

Let's dive into how you can leverage tools with [AI TOOLKIT RSC](/docs/ai-toolkit-rsc/overview) to build a generative user interface with Next.js (App Router).
...
To see AI TOOLKIT RSC in action, check out our open-source [Next.js Gemini Chatbot](https://gemini.vercel.ai/).

## Migrate from OpenAI

One of the key advantages of the AI TOOLKIT is its unified API, which makes it incredibly easy to switch between different AI models and providers. This flexibility is particularly useful when you want to migrate from one model to another, such as moving from OpenAI's GPT models to Meta's Llama models hosted on DeepInfra.
...
With just these few changes, you've migrated from using OpenAI's GPT-4-Turbo to Meta's Llama 3.1 hosted on DeepInfra. The `generateText` function and its usage remain identical, showcasing the power of the AI TOOLKIT's unified API.
...
The AI TOOLKIT offers a powerful and flexible way to integrate cutting-edge AI models like Llama 3.1 into your applications. With AI TOOLKIT Core, you can seamlessly switch between different AI models and providers by changing just two lines of code. This flexibility allows for quick experimentation and adaptation, reducing the time required to change models from days to minutes.

The AI TOOLKIT ensures that your application remains clean and modular, accelerating development and future-proofing against the rapidly evolving landscape.

Ready to get started? Here's how you can dive in:

1. Explore the documentation at [studio.khulnasoft.com/docs](/docs) to understand the full capabilities of the AI TOOLKIT.
2. Check out practical examples at [studio.khulnasoft.com/examples](/examples) to see the SDK in action and get inspired for your own projects.
3. Dive deeper with advanced guides on topics like Retrieval-Augmented Generation (RAG) and multi-modal chat at [studio.khulnasoft.com/docs/guides](/docs/guides).
