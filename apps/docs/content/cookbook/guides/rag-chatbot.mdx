---
title: RAG Agent
description: Learn how to build a RAG Agent with the AI TOOLKIT and Next.js
tags:
  [
    'rag',
    'chatbot',
...
[Embeddings](/docs/ai-toolkit-core/embeddings) are a way to represent words, phrases, or images as vectors in a high-dimensional space. In this space, similar words are close to each other, and the distance between words can be used to measure their similarity.
...
- [ AI TOOLKIT ](/docs)
- [ Vercel AI Gateway ](/providers/ai-toolkit-providers/ai-gateway)
...
To reduce the scope of this guide, you will be starting with a [repository](https://github.com/vercel/ai-toolkit-rag-starter) that already has a few things set up for you:
...
<Snippet
  text={[
    'git clone https://github.com/vercel/ai-toolkit-rag-starter',
    'cd ai-toolkit-rag-starter',
  ]}
/>
...
```tsx title="lib/db/schema/embeddings.ts"
...
You will use the AI TOOLKIT to create embeddings. This will require two more dependencies, which you can install by running the following command:

<Snippet text="pnpm add ai @ai-toolkit/react" />

This will install the [AI TOOLKIT](/docs) and the AI TOOLKIT's React hooks.
...
```tsx title="lib/ai/embedding.ts" {1-2,4,13-22}
...
```tsx title="lib/actions/resources.ts"
'use server';
...
```tsx title="lib/actions/resources.ts" {9-10,21-27,29}
'use server';
...
Great! Let's build the frontend. The AI TOOLKIT’s [`useChat`](/docs/reference/ai-toolkit-ui/use-chat) hook allows you to easily create a conversational user interface for your agent.
...
In this code, you declare and export an asynchronous function called POST. You retrieve the `messages` from the request body and then pass them to the [`streamText`](/docs/reference/ai-toolkit-core/stream-text) function imported from the AI TOOLKIT, alongside the model you would like to use. Finally, you return the model’s response in `UIMessageStreamResponse` format.
...
```tsx title="app/api/chat/route.ts" {12-14}
import { convertToModelMessages, streamText, UIMessage } from 'ai-toolkit';
...
```tsx title="app/api/chat/route.ts" {18-29}
import { createResource } from '@/lib/actions/resources';
import { convertToModelMessages, streamText, tool, UIMessage } from 'ai-toolkit';
import { z } from 'zod';
...
In simple terms, on each generation, the model will decide whether it should call the tool. If it deems it should call the tool, it will extract the input and then append a new `message` to the `messages` array of type `tool-call`. The AI TOOLKIT will then run the `execute` function with the parameters provided by the `tool-call` message.
...
The AI TOOLKIT has a feature called [`stopWhen`](/docs/ai-toolkit-core/tools-and-tool-calling#multi-step-calls-using-stopwhen) which allows stopping conditions when the model generates a tool call. If those stopping conditions haven't been hit, the AI TOOLKIT will automatically send tool call results back to the model!

Open your root page (`api/chat/route.ts`) and add the following key to the `streamText` configuration object:

```tsx title="api/chat/route.ts" {8,24}
import { createResource } from '@/lib/actions/resources';
import {
...
```tsx title="lib/ai/embedding.ts" {1,3-5,27-34,36-49}
...
```ts title="api/chat/route.ts" {11,37-43}
import { createResource } from '@/lib/actions/resources';
import {
  convertToModelMessages,
  streamText,
...
[More info](https://github.com/vercel/ai-toolkit-rag-starter/issues/1).
