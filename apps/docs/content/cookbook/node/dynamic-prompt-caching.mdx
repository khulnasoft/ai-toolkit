...
You might notice this implementation adds `providerOptions` at the **message level**, while Anthropic's API expects `cache_control` at the **content block level**. The AI TOOLKIT handles this translation automatically.
...
```ts
import type { ModelMessage, JSONValue, LanguageModel } from 'ai-toolkit';
