...
<Note>
  The `/edge` sub-module is included in the `@ai-toolkit/google-vertex` package,
  so you don't need to install it separately. You must import from
  `@ai-toolkit/google-vertex/edge` to differentiate it from the Node.js
  provider.
</Note>
...
```ts
import { vertex } from '@ai-toolkit/google-vertex';
import { GoogleGenerativeAIProviderOptions } from '@ai-toolkit/google'; // Note: importing from @ai-toolkit/google
...
<Note>
  ...
  except for `gs://` URLs. You can use the Google Cloud Storage API to upload
  larger files to that location.
</Note>
...
using the `.image()` factory method. For more on image generation with the AI TOOLKIT see [generateImage()](/docs/reference/ai-toolkit-core/generate-image).
...
```ts
import { vertex } from '@ai-toolkit/google-vertex';
import { GoogleVertexImageProviderOptions } from '@ai-toolkit/google-vertex';
import { generateImage } from 'ai-toolkit';
