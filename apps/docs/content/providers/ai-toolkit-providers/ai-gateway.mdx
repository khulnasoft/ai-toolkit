---
title: AI Gateway
description: Learn how to use the AI Gateway provider with the AI TOOLKIT.
---
...
The Vercel AI Gateway provider is part of the AI TOOLKIT.
...
import { generateText, gateway } from 'ai-toolkit';
...
The AI TOOLKIT automatically uses the AI Gateway when you pass a model string in the `creator/model-name` format.
...
- Use the provider in a [provider registry](/docs/ai-toolkit-core/provider-management)
- Wrap the provider with [middleware](/docs/ai-toolkit-core/middleware)
...
```ts
import { createGateway } from 'ai-toolkit';
...
AI Gateway language models can also be used in the `streamText`, `generateObject`, and `streamObject` functions (see [AI TOOLKIT Core](/docs/ai-toolkit-core)).
...
Some providers offer tools that are executed by the provider itself, such as [OpenAI's web search tool](/providers/ai-toolkit-providers/openai#web-search-tool). To use these tools through AI Gateway, import the provider to access the tool definitions:
...
```ts
import type { AnthropicProviderOptions } from '@ai-toolkit/anthropic';
import type { GatewayProviderOptions } from '@ai-toolkit/gateway';
import { generateText } from 'ai-toolkit';
...
This works with any provider supported by AI Gateway. Each provider has its own set of options - see the individual [provider documentation pages](/providers/ai-toolkit-providers) for details on provider-specific options.
...
- Individual [AI TOOLKIT provider pages](/providers/ai-toolkit-providers) for specific model capabilities and features
