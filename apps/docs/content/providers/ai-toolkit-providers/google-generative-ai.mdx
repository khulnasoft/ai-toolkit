...
<Note>
  ...
  except for `https://generativelanguage.googleapis.com/v1beta/files/` and
  YouTube URLs. You can use the Google Generative AI Files API to upload larger
  files to that location. YouTube URLs (public or unlisted videos) are supported
  directly - you can specify one YouTube video URL per request.
</Note>
...
<Note>
  Usage metadata was added to `providerMetadata` in `@ai-toolkit/google@1.2.23`.
  If you are using an older version, usage metadata is available in the raw HTTP
...
```json
{
  "urlMetadata": [
    {
      "retrievedUrl": "https://studio.khulnasoft.com/providers/ai-toolkit-providers/google-generative-ai",
...
"groundingMetadata": {
    "groundingChunks": [
        {
            "web": {
                "uri": "https://studio.khulnasoft.com/providers/ai-toolkit-providers/google-generative-ai",
                "title": "Google Generative AI - AI TOOLKIT Providers"
            }
        }
    ],
    "groundingSupports": [
...
const { text, sources, providerMetadata } = await generateText({
  model: google('gemini-2.5-flash'),
  prompt: `Based on this context: https://studio.khulnasoft.com/providers/ai-toolkit-providers/google-generative-ai, tell me how to use Gemini with AI TOOLKIT.
...
<Note>
  ...
  the Google Vertex provider (`@ai-toolkit/google-vertex`) instead of the
  standard Google provider (`@ai-toolkit/google`) to use this feature.
</Note>
...
```ts
import { google } from '@ai-toolkit/google';
import { GoogleGenerativeAIImageProviderOptions } from '@ai-toolkit/google';
import { generateImage } from 'ai-toolkit';
