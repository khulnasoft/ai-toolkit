...
<Note>
  ...
  docs](https://docs.lmnr.ai/tracing/integrations/vercel-ai-toolkit).
</Note>

## Setup

Laminar's tracing is based on OpenTelemetry. It supports AI TOOLKIT [telemetry](/docs/ai-toolkit-core/telemetry).
...
### Tracing AI TOOLKIT calls

Then, when you call AI TOOLKIT functions in any of your API routes, add the Laminar tracer to the `experimental_telemetry` option.
...
Laminar can live alongside `@vercel/otel` and trace AI TOOLKIT calls. The default Laminar setup will ensure that

- regular Next.js traces are sent via `@vercel/otel` to your Telemetry backend configured with Vercel,
- AI TOOLKIT and other LLM or browser agent traces are sent via Laminar.
...
Laminar can live alongside `@sentry/node` and trace AI TOOLKIT calls. Make sure to initialize Laminar **after** `Sentry.init`.
...
Laminar can work with `@sentry/node` to trace AI TOOLKIT calls. Make sure to initialize Laminar **after** `Sentry.init`:
