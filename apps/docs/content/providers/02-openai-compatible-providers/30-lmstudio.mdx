---
title: LM Studio
description: Use the LM Studio OpenAI compatible API with the AI TOOLKIT.
---
...
It contains an OpenAI compatible API server that you can use with the AI TOOLKIT.
...
The LM Studio provider is available via the `@ai-toolkit/openai-compatible` module as it is compatible with the OpenAI API.
...
To use LM Studio, you can create a custom provider instance with the `createOpenAICompatible` function from `@ai-toolkit/openai-compatible`:
