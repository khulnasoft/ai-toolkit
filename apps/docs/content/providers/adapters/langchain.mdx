...
The `@ai-toolkit/langchain` adapter provides seamless integration between LangChain, LangGraph, and the AI TOOLKIT,
enabling you to use LangChain models and LangGraph agents with AI TOOLKIT UI components.

## Installation

<Tabs items={['pnpm', 'npm', 'yarn']}>
  <Tab>
    <Snippet text="pnpm add @ai-toolkit/langchain @langchain/core" dark />
  </Tab>
  <Tab>
    <Snippet text="npm install @ai-toolkit/langchain @langchain/core" dark />
  </Tab>
  <Tab>
    <Snippet text="yarn add @ai-toolkit/langchain @langchain/core" dark />
  </Tab>
</Tabs>
...
- Convert AI TOOLKIT `UIMessage` to LangChain `BaseMessage` format using `toBaseMessages`
- Transform LangChain/LangGraph streams to AI TOOLKIT `UIMessageStream` using `toUIMessageStream`
...
Then, use the AI TOOLKIT's [`useChat`](/docs/ai-toolkit-ui/chatbot) hook in the page component:
...
```tsx title="app/api/langgraph/route.ts"
import { toBaseMessages, toUIMessageStream } from '@ai-toolkit/langchain';
import { ChatOpenAI } from '@langchain/openai';
import { createUIMessageStreamResponse, UIMessage } from 'ai-toolkit';
...
```tsx title="app/api/stream-events/route.ts"
import { toBaseMessages, toUIMessageStream } from '@ai-toolkit/langchain';
import { ChatOpenAI } from '@langchain/openai';
import { createUIMessageStreamResponse, UIMessage } from 'ai-toolkit';
...
```tsx title="app/api/custom-data/route.ts"
import { createUIMessageStreamResponse, UIMessage } from 'ai-toolkit';
...
```tsx title="app/langsmith/page.tsx"
'use client';
...
Converts AI TOOLKIT `ModelMessage` objects to LangChain `BaseMessage` objects. Useful when you already have model messages from `convertToModelMessages`.

```ts
import { convertModelMessages } from '@ai-toolkit/langchain';
...
Converts a LangChain/LangGraph stream to an AI TOOLKIT `UIMessageStream`. Automatically detects the stream type and handles direct model streams, LangGraph streams, and `streamEvents()` output.
...
You can find additional examples in the AI TOOLKIT [examples/next-langchain](https://github.com/khulnasoft/ai-toolkit/tree/main/examples/next-langchain) folder.
