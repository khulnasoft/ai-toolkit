...
- It is not possible to abort a stream using server actions. This will be improved in future releases of React and Next.js [(1122)](https://github.com/khulnasoft/ai-toolkit/issues/1122).
- When using `createStreamableUI` and `streamUI`, components remount on `.done()`, causing them to flicker [(2939)](https://github.com/khulnasoft/ai-toolkit/issues/2939).
- Many suspense boundaries can lead to crashes [(2843)](https://github.com/khulnasoft/ai-toolkit/issues/2843).
...
- Closed RSC streams cause update issues [(3007)](https://github.com/khulnasoft/ai-toolkit/issues/3007).
...
```tsx title="@/app/components/list-flights.tsx"
'use client';
...
```tsx title="@/app/components/message.tsx"
'use client';
...
import { Message } from 'ai-toolkit';
import { useChat } from '@ai-toolkit/react';
...
```ts title="@/app/api/object/route.ts"
import { streamObject } from 'ai-toolkit';
import { openai } from '@ai-toolkit/openai';
