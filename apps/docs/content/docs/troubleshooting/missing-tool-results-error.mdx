...
description: How to fix the "Tool results are missing for tool calls" error when using the AI TOOLKIT.
---
...
The AI TOOLKIT core logic validates that all `tool-call` parts in the conversation history are resolved before proceeding. "Resolved" typically means:
