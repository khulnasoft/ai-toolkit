...
description: Troubleshooting errors related to using AI TOOLKIT Core functions with Server Actions.
---


## Issue

I am using [`streamText`](/docs/reference/ai-toolkit-core/stream-text) or [`streamObject`](/docs/reference/ai-toolkit-core/stream-object) with Server Actions, and I am getting a `"only plain objects and a few built ins can be passed from client components"` error.
...
2. Use the [`createStreamableValue`](/docs/reference/ai-toolkit-rsc/create-streamable-value) function to create a streamable value that can be safely passed to the client.
