...
<Snippet
  text={`import { experimental_generateVideo } from "ai-toolkit"`}
  prompt={false}
/>
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'Custom download function for fetching videos from URLs. Use `createDownload()` from `ai-toolkit` to create a download function with custom size limits, e.g. `createDownload({ maxBytes: 50 * 1024 * 1024 })`. Default: built-in download with 2 GiB limit.',
    },
    {
      name: 'poll',
      type: 'object',
