...
The `createAgentUIStreamResponse` function executes an [Agent](/docs/reference/ai-toolkit-core/agent), runs its streaming output as a UI message stream, and returns an HTTP [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response) object whose body is the live, streaming UI message output. This is designed for API routes that deliver real-time agent results, such as chat endpoints or streaming tool-use operations.

## Import

<Snippet
  text={`import { createAgentUIStreamResponse } from "ai-toolkit"`}
  prompt={false}
/>
...
```ts
import { ToolLoopAgent, createAgentUIStreamResponse } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isRequired: false,
      description:
        'Other UI message output options—such as `sendSources`, `includeUsage`, and more. See [`UIMessageStreamOptions`](/docs/reference/ai-toolkit-core/ui-message-stream-options).',
    },
    {
      name: 'headers',
      type: 'HeadersInit',
