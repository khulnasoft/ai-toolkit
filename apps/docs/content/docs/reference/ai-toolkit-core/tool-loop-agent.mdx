...
<Snippet text={`import { ToolLoopAgent } from "ai-toolkit"`} prompt={false} />
...
The `generate()` method returns a `GenerateTextResult` object (see [`generateText`](/docs/reference/ai-toolkit-core/generate-text#returns) for details).
...
The `stream()` method returns a `StreamTextResult` object (see [`streamText`](/docs/reference/ai-toolkit-core/stream-text#returns) for details).
