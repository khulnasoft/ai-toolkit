...
Implementations of the `Agent` interface—such as `ToolLoopAgent`—fulfill the same contract and integrate seamlessly with all SDK APIs and utilities that expect an agent. This design allows users to supply custom agent classes or wrappers for third-party chains, while maximizing compatibility with AI TOOLKIT features.
...
```ts
import { Agent, GenerateTextResult, StreamTextResult } from 'ai-toolkit';
...
All SDK utilities that accept an agent—including [`createAgentUIStream`](/docs/reference/ai-toolkit-core/create-agent-ui-stream), [`createAgentUIStreamResponse`](/docs/reference/ai-toolkit-core/create-agent-ui-stream-response), and [`pipeAgentUIStreamToResponse`](/docs/reference/ai-toolkit-core/pipe-agent-ui-stream-to-response)—expect an object adhering to the `Agent` interface.

You can use the official [`ToolLoopAgent`](/docs/reference/ai-toolkit-core/tool-loop-agent) (recommended for multi-step AI workflows with tool use), or supply your own implementation:
...
- [`ToolLoopAgent`](/docs/reference/ai-toolkit-core/tool-loop-agent) &mdash; Official multi-step agent implementation
- [`createAgentUIStream`](/docs/reference/ai-toolkit-core/create-agent-ui-stream)
- [`GenerateTextResult`](/docs/reference/ai-toolkit-core/generate-text)
- [`StreamTextResult`](/docs/reference/ai-toolkit-core/stream-text)
