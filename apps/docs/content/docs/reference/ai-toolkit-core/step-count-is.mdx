...
<Snippet text={`import { stepCountIs } from "ai-toolkit"`} prompt={false} />
...
```ts
import { generateText, stepCountIs, hasToolCall } from 'ai-toolkit';
...
- [`hasToolCall()`](/docs/reference/ai-toolkit-core/has-tool-call)
