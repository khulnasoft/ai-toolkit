...
<Note type="warning">
  `streamToResponse` has been removed in AI TOOLKIT 4.0. Use
  `pipeDataStreamToResponse` from
  [streamText](/docs/reference/ai-toolkit-core/stream-text) instead.
</Note>
...
<Snippet text={`import { streamToResponse } from "ai-toolkit"`} prompt={false} />
...
```ts
import { StreamData, streamText, streamToResponse } from 'ai-toolkit';
__PROVIDER_IMPORT__;
