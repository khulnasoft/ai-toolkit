---
title: '@ai-toolkit/llamaindex Adapter'
description: API Reference for the LlamaIndex Adapter.
---


The `@ai-toolkit/llamaindex` package provides helper functions to transform LlamaIndex output streams into data streams and data stream responses.
...
<Snippet
  text={`import { toDataResponse } from "@ai-toolkit/llamaindex"`}
  prompt={false}
/>
...
```tsx title="app/api/completion/route.ts" {15}
