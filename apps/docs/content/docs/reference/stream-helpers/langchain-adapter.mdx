---
title: '@ai-toolkit/langchain Adapter'
description: API Reference for the LangChain Adapter.
---


The `@ai-toolkit/langchain` module provides helper functions to transform LangChain output streams into data streams and data stream responses.
...
<Snippet
  text={`import { toDataStreamResponse } from "@ai-toolkit/langchain"`}
  prompt={false}
/>
...
```tsx title="app/api/completion/route.ts" {13}
