...
<Note>
  The `useChat` API has been significantly updated in AI TOOLKIT 5.0. It now
  uses a transport-based architecture and no longer manages input state
  internally. See the [migration
...
<Tabs items={['React', 'Svelte', 'Vue']}>
  ...
  <Tab>
    <Snippet
      text="import { Chat } from '@ai-toolkit/svelte'"
      dark
      prompt={false}
    />
  </Tab>
  <Tab>
    <Snippet
      text="import { Chat } from '@ai-toolkit/vue'"
      dark
      prompt={false}
    />
  </Tab>
