...
A transport that directly communicates with an [Agent](/docs/reference/ai-toolkit-core/agent) in-process, without going through HTTP. This is useful for:
...
<Snippet text={`import { DirectChatTransport } from "ai-toolkit"`} prompt={false} />
...
```tsx
import { useChat } from '@ai-toolkit/react';
import { DirectChatTransport, ToolLoopAgent, tool } from 'ai-toolkit';
import { openai } from '@ai-toolkit/openai';
import { z } from 'zod';
