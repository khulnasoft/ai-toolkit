...
By default, agents run for 20 steps (`stopWhen: stepCountIs(20)`). In each step, the model either generates text or calls a tool. If it generates text, the agent completes. If it calls a tool, the AI TOOLKIT executes that tool.
...
```ts
import { ToolLoopAgent, Output, stepCountIs } from 'ai-toolkit';
__PROVIDER_IMPORT__;
import { z } from 'zod';
