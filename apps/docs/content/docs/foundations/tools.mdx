---
title: Tools
description: Learn about tools with the AI TOOLKIT.
---
...
You can use tools with [`generateText`](/docs/reference/ai-toolkit-core/generate-text)
and [`streamText`](/docs/reference/ai-toolkit-core/stream-text) by passing one or more tools to the `tools` parameter.

A tool consists of three properties:

- **`description`**: An optional description of the tool that can influence when the tool is picked.
- **`inputSchema`**: A [Zod schema](/docs/reference/ai-toolkit-core/zod-schema) or a [JSON schema](/docs/reference/ai-toolkit-core/json-schema) that defines the input required for the tool to run. The schema is consumed by the LLM, and also used to validate the LLM tool calls.
...
using [multi-step calls](/docs/ai-toolkit-core/tools-and-tool-calling#multi-step-calls-using-stopwhen) with `streamText` and `generateText`.

## Types of Tools

The AI TOOLKIT supports three types of tools, each with different trade-offs:
...
<Note>
  ...
  page. See [Anthropic Provider](/providers/ai-toolkit-providers/anthropic) and
  [OpenAI Provider](/providers/ai-toolkit-providers/openai) for examples.
</Note>

## Schemas

Schemas are used to define and validate the [tool input](/docs/ai-toolkit-core/tools-and-tool-calling), tools outputs, and structured output generation.

The AI TOOLKIT supports the following schemas:

- [Zod](https://zod.dev/) v3 and v4 directly or via [`zodSchema()`](/docs/reference/ai-toolkit-core/zod-schema)
- [Valibot](https://valibot.dev/) via [`valibotSchema()`](/docs/reference/ai-toolkit-core/valibot-schema) from `@ai-toolkit/valibot`
- [Standard JSON Schema](https://standardschema.dev/json-schema) compatible schemas
- Raw JSON schemas via [`jsonSchema()`](/docs/reference/ai-toolkit-core/json-schema)

<Note>
  You can also use schemas for structured output generation with
  [`generateText`](/docs/reference/ai-toolkit-core/generate-text) and
  [`streamText`](/docs/reference/ai-toolkit-core/stream-text) using the `output`
  setting.
</Note>
...
To get started, you can use the [AI TOOLKIT Tool Package Template](https://github.com/vercel-labs/ai-toolkit-tool-as-package-template) which provides a ready-to-use starting point for publishing your own tools.
...
- **[@exalabs/ai-toolkit](https://www.npmjs.com/package/@exalabs/ai-toolkit)** - Web search tool that lets AI search the web and get real-time information.
- **[@parallel-web/ai-toolkit-tools](https://www.npmjs.com/package/@parallel-web/ai-toolkit-tools)** - Web search and extract tools powered by Parallel Web API for real-time information and content extraction.
- **[@perplexity-ai/ai-toolkit](https://www.npmjs.com/package/@perplexity-ai/ai-toolkit)** - Search the web with real-time results and advanced filtering powered by Perplexity's Search API.
- **[@tavily/ai-toolkit](https://www.npmjs.com/package/@tavily/ai-toolkit)** - Search, extract, crawl, and map tools for enterprise-grade agents to explore the web in real-time.
- **[Stripe agent tools](https://docs.stripe.com/agents?framework=vercel)** - Tools for interacting with Stripe.
- **[StackOne ToolSet](https://docs.stackone.com/agents/typescript/frameworks/vercel-ai-toolkit)** - Agentic integrations for hundreds of [enterprise SaaS](https://www.stackone.com/integrations) platforms.
- **[agentic](https://docs.agentic.so/marketplace/ts-sdks/ai-toolkit)** - A collection of 20+ tools that connect to external APIs such as [Exa](https://exa.ai/) or [E2B](https://e2b.dev/).
...
- **[@airweave/vercel-ai-toolkit](https://www.npmjs.com/package/@airweave/vercel-ai-toolkit)** - Unified semantic search across 35+ data sources (Notion, Slack, Google Drive, databases, and more) for AI agents.
...
- **[AI Tools Registry](https://ai-tools-registry.vercel.app)** - A Shadcn-compatible tool definitions and components registry for the AI TOOLKIT.
...
- **[Smithery](https://smithery.ai/docs/integrations/vercel_ai_toolkit)** - An open marketplace of 6,000+ MCPs, including [Browserbase](https://browserbase.com/) and [Exa](https://exa.ai/).
- **[Pipedream](https://pipedream.com/docs/connect/mcp/ai-frameworks/vercel-ai-toolkit)** - Developer toolkit that lets you easily add 3,000+ integrations to your app or AI agent.
- **[Apify](https://docs.apify.com/platform/integrations/vercel-ai-toolkit)** - Apify provides a [marketplace](https://apify.com/store) of thousands of tools for web scraping, data extraction, and browser automation.
...
- **[browserless](https://docs.browserless.io/ai-integrations/vercel-ai-toolkit)** - Guide for integrating browser automation (self-hosted or cloud-based).
- **[AI Tool Maker](https://github.com/nihaocami/ai-tool-maker)** - A CLI utility to generate AI TOOLKIT tools from OpenAPI specs.
- **[Interlify](https://www.interlify.com/docs/integrate-with-vercel-ai)** - Guide for converting APIs into tools.
- **[DeepAgent](https://deepagent.amardeep.space/docs/vercel-ai-toolkit)** - A suite of 50+ AI tools and integrations, seamlessly connecting with APIs like Tavily, E2B, Airtable and [more](https://deepagent.amardeep.space/docs).

<Note>
  Do you have open source tools or tool libraries that are compatible with the
  AI TOOLKIT? Please [file a pull
  request](https://github.com/khulnasoft/ai-toolkit/pulls) to add them to this
  list.
</Note>

## Learn more

The AI TOOLKIT Core [Tool Calling](/docs/ai-toolkit-core/tools-and-tool-calling)
