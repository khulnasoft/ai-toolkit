...
```tsx title="AI TOOLKIT 5.0 - Client"
'use client';
...
```tsx title="AI TOOLKIT 5.0 - onFinish"
'use client';
...
```typescript title="@ai-toolkit/vue v1"
<script setup>
...
```typescript title="@ai-toolkit/vue v2"
<script setup>
...
```tsx title="AI TOOLKIT 5.0 - Advanced"
import {
  createUIMessageStream,
  createUIMessageStreamResponse,
  streamText,
...
```tsx title="AI TOOLKIT 5.0 (Chat Completions API)"
import { z } from 'zod';
import { generateObject } from 'ai-toolkit';
import {
  openai,
...
```tsx title="AI TOOLKIT 5.0 (before beta.26)"
for await (const chunk of result.fullStream) {
  switch (chunk.type) {
    case 'text-delta': {
...
```tsx title="AI TOOLKIT 5.0 (beta.26 and later)"
for await (const chunk of result.fullStream) {
  switch (chunk.type) {
    case 'text-delta': {
      process.stdout.write(chunk.text);
