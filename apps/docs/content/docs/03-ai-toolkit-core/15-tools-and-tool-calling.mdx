---
title: Tool Calling
description: Learn about tool calling and multi-step calls (using stopWhen) with AI TOOLKIT Core.
---
...
AI TOOLKIT Core tools contain several core elements:

- **`description`**: An optional description of the tool that can influence when the tool is picked.
- **`inputSchema`**: A [Zod schema](/docs/foundations/tools#schemas) or a [JSON schema](/docs/reference/ai-toolkit-core/json-schema) that defines the input parameters. The schema is consumed by the LLM, and also used to validate the LLM tool calls.
...
<Note className="mb-2">
  You can use the [`tool`](/docs/reference/ai-toolkit-core/tool) helper function
  to infer the types of the `execute` parameters.
</Note>
...
```ts
import { type ModelMessage, generateText } from 'ai-toolkit';
...
```ts
import { type ToolApprovalResponse } from 'ai-toolkit';
...
When using `useChat`, the approval flow is handled through UI state. See [Chatbot Tool Usage](/docs/ai-toolkit-ui/chatbot-tool-usage#tool-execution-approval) for details on handling approvals in your UI with `addToolApprovalResponse`.

## Multi-Step Calls (using stopWhen)

With the `stopWhen` setting, you can enable multi-step calls in `generateText` and `streamText`. When `stopWhen` is set and the model generates a tool call, the AI TOOLKIT will trigger a new generation passing in the tool result until there are no further tool calls or the stopping condition is met.
...
```tsx
import { forwardAnthropicContainerIdFromLastStep } from '@ai-toolkit/anthropic';
...
```ts
import { generateText, ModelMessage } from 'ai-toolkit';
...
AI TOOLKIT Core supports dynamic tools for scenarios where tool schemas are not known at compile time. This is useful for:
...
To enable this, the AI TOOLKIT provides several helper types for tools, tool calls, and tool results.
...
import { TypedToolCall, TypedToolResult, generateText, tool } from 'ai-toolkit';
__PROVIDER_IMPORT__;
import { z } from 'zod';
...
The AI TOOLKIT has three tool-call related errors:

- [`NoSuchToolError`](/docs/reference/ai-toolkit-errors/ai-no-such-tool-error): the model tries to call a tool that is not defined in the tools object
- [`InvalidToolInputError`](/docs/reference/ai-toolkit-errors/ai-invalid-tool-input-error): the model calls a tool with inputs that do not match the tool's input schema
- [`ToolCallRepairError`](/docs/reference/ai-toolkit-errors/ai-tool-call-repair-error): an error that occurred during tool call repair

When tool execution fails (errors thrown by your tool's `execute` function), the AI TOOLKIT adds them as `tool-error` content parts to enable automated LLM roundtrips in multi-step scenarios.
...
the AI TOOLKIT provides the `activeTools` property.
...
AI TOOLKIT Core tools have an optional `toModelOutput` function
...
The AI TOOLKIT supports connecting to Model Context Protocol (MCP) servers to access their tools.
MCP enables your AI applications to discover and use tools across various services through a standardized interface.

For detailed information about MCP tools, including initialization, transport options, and usage patterns, see the [MCP Tools documentation](/docs/ai-toolkit-core/mcp-tools).

### AI TOOLKIT Tools vs MCP Tools

In most cases, you should define your own AI TOOLKIT tools for production applications. They provide full control, type safety, and optimal performance. MCP tools are best suited for rapid development iteration and scenarios where users bring their own tools.

| Aspect                 | AI TOOLKIT Tools                                          | MCP Tools                                             |
| ---------------------- | --------------------------------------------------------- | ----------------------------------------------------- |
