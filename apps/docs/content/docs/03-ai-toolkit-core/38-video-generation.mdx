...
description: Learn how to generate videos with the AI TOOLKIT.
---
...
The AI TOOLKIT provides the [`experimental_generateVideo`](/docs/reference/ai-toolkit-core/generate-video)
...
Each video model has an internal limit on how many videos it can generate in a single API call. The AI TOOLKIT manages this automatically by batching requests appropriately when you request multiple videos using the `n` parameter. Most video models only support generating 1 video per call due to computational cost.
...
Video generation is an asynchronous process that can take several minutes to complete. The AI TOOLKIT automatically polls the provider to check if the video is ready. You can configure the polling behavior:
...
`headers` and `body`. The AI TOOLKIT sends `url` to the provider, waits for `received`,
and then retrieves the completed video.

You can provide `poll` together with `webhook`. For models with native webhook
support, `poll.timeoutMs` limits how long the AI TOOLKIT waits for the notification.
If the model does not support webhooks, the AI TOOLKIT falls back to polling with the
...
When `experimental_generateVideo` cannot generate a valid video, it throws a [`AI_NoVideoGeneratedError`](/docs/reference/ai-toolkit-errors/ai-no-video-generated-error).
...
| ----------------------------------------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------ |
| [ByteDance](/providers/ai-toolkit-providers/bytedance#video-models)                 | `seedance-1-0-pro-250528`   | Text-to-video, image-to-video, editing, extension                              |
| [Black Forest Labs](/providers/ai-toolkit-providers/black-forest-labs#video-models) | `flux-3-video`              | Text-to-video, image-to-video, keyframes, video continuation, audio generation |
| [FAL](/providers/ai-toolkit-providers/fal#video-models)                             | `luma-dream-machine/ray-2`  | Text-to-video, image-to-video                                                  |
| [FAL](/providers/ai-toolkit-providers/fal#video-models)                             | `minimax-video`             | Text-to-video                                                                  |
| [Google](/providers/ai-toolkit-providers/google-generative-ai#video-models)         | `veo-2.0-generate-001`      | Text-to-video, up to 4 videos per call                                         |
| [Google Vertex](/providers/ai-toolkit-providers/google-vertex#video-models)         | `veo-3.1-generate-001`      | Text-to-video, audio generation                                                |
| [Google Vertex](/providers/ai-toolkit-providers/google-vertex#video-models)         | `veo-3.1-fast-generate-001` | Text-to-video, audio generation                                                |
| [Google Vertex](/providers/ai-toolkit-providers/google-vertex#video-models)         | `veo-3.0-generate-001`      | Text-to-video, audio generation                                                |
| [Google Vertex](/providers/ai-toolkit-providers/google-vertex#video-models)         | `veo-3.0-fast-generate-001` | Text-to-video, audio generation                                                |
| [Google Vertex](/providers/ai-toolkit-providers/google-vertex#video-models)         | `veo-2.0-generate-001`      | Text-to-video, up to 4 videos per call                                         |
| [Replicate](/providers/ai-toolkit-providers/replicate#video-models)                 | `minimax/video-01`          | Text-to-video                                                                  |
| [xAI](/providers/ai-toolkit-providers/xai#video-models)                             | `grok-imagine-video`        | Text-to-video, image-to-video, editing, extension, R2V                         |
| [xAI](/providers/ai-toolkit-providers/xai#video-models)                             | `grok-imagine-video-1.5`    | Text-to-video, image-to-video, editing, extension, R2V (with reference audio)  |

Above are a small subset of the video models supported by the AI TOOLKIT providers. For more, see the respective provider documentation.
