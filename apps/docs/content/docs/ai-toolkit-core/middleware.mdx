...
```ts
import { wrapLanguageModel, extractReasoningMiddleware } from 'ai-toolkit';
...
```ts
import { wrapLanguageModel, extractJsonMiddleware, Output } from 'ai-toolkit';
import { z } from 'zod';
...
```ts
import { wrapLanguageModel, simulateStreamingMiddleware } from 'ai-toolkit';
...
```ts
import { wrapLanguageModel, defaultSettingsMiddleware } from 'ai-toolkit';
...
The AI TOOLKIT provides a Language Model Middleware specification. Community members can develop middleware that adheres to this specification, making it compatible with the AI TOOLKIT ecosystem.

Here are some community middlewares that you can explore:

### Custom tool call parser

The [Custom tool call parser](https://github.com/minpeter/ai-toolkit-tool-call-middleware) middleware extends tool call capabilities to models that don't natively support the OpenAI-style `tools` parameter. This includes many self-hosted and third-party models that lack native function calling features.
...
The `@ai-toolkit-tool/parser` package offers three middleware variants:
...
```ts
import { wrapLanguageModel } from 'ai-toolkit';
import { gemmaToolMiddleware } from '@ai-toolkit-tool/parser';
...
Find more examples at this [link](https://github.com/minpeter/ai-toolkit-tool-call-middleware/tree/main/examples/core/src).
...
<Note>
  ...
  specification](https://github.com/khulnasoft/ai-toolkit/blob/v5/packages/provider/src/language-model/v2/language-model-v2.ts).
</Note>
...
2. `wrapGenerate`: Wraps the `doGenerate` method of the [language model](https://github.com/khulnasoft/ai-toolkit/blob/v5/packages/provider/src/language-model/v2/language-model-v2.ts).
   You can modify the parameters, call the language model, and modify the result.
3. `wrapStream`: Wraps the `doStream` method of the [language model](https://github.com/khulnasoft/ai-toolkit/blob/v5/packages/provider/src/language-model/v2/language-model-v2.ts).
   You can modify the parameters, call the language model, and modify the result.
