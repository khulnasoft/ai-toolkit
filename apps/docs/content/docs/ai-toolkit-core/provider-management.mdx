...
You can mix and match custom providers, the provider registry, and [middleware](/docs/ai-toolkit-core/middleware) in your application.
...
```ts
import { customProvider, gateway } from 'ai-toolkit';
...
You can create a [provider registry](/docs/reference/ai-toolkit-core/provider-registry) with multiple providers and models using `createProviderRegistry`.
...
import { createProviderRegistry, gateway } from 'ai-toolkit';
...
The AI TOOLKIT 5 includes a global provider feature that allows you to specify a model using just a plain model ID string:
