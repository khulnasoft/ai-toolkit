...
2. The [`sendAutomaticallyWhen`](/docs/reference/ai-toolkit-ui/use-chat#send-automatically-when) option with `lastAssistantMessageIsCompleteWithToolCalls` helper automatically submits when all tool results are available.
...
```tsx title='app/page.tsx' {2,6,10,14-20}
'use client';
...
```tsx title='app/page.tsx' {19,36-41}
'use client';
...
Enable approval by setting `needsApproval` on your tool. See [Tool Execution Approval](/docs/ai-toolkit-core/tools-and-tool-calling#tool-execution-approval) for configuration options including dynamic approval based on input.
...
```tsx
import { useChat } from '@ai-toolkit/react';
import { lastAssistantMessageIsCompleteWithApprovalResponses } from 'ai-toolkit';
...
Tool call streaming is **enabled by default** in AI TOOLKIT 5.0, allowing you to stream tool calls while they are being generated. This provides a better user experience by showing tool inputs as they are generated in real-time.
...
```tsx title='app/api/chat/route.ts' {15-21,24}
import { convertToModelMessages, streamText, UIMessage, stepCountIs } from 'ai-toolkit';
__PROVIDER_IMPORT__;
import { z } from 'zod';
