...
description: Learn how to attach and use metadata with messages in AI TOOLKIT UI
---
...
Message metadata differs from [data parts](/docs/ai-toolkit-ui/streaming-data) in that it's attached at the message level rather than being part of the message content. While data parts are ideal for dynamic content that forms part of the message, metadata is perfect for information about the message itself.
...
```ts title="app/api/chat/route.ts" {11-20}
...
```tsx title="app/page.tsx" {8,18-23}
'use client';
...
<Note>
  ...
  [data parts](/docs/ai-toolkit-ui/streaming-data) instead.
</Note>
...
- [Chatbot Guide](/docs/ai-toolkit-ui/chatbot#message-metadata) - Message metadata in the context of building chatbots
- [Streaming Data](/docs/ai-toolkit-ui/streaming-data#message-metadata-vs-data-parts) - Comparison with data parts
- [UIMessage Reference](/docs/reference/ai-toolkit-core/ui-message) - Complete UIMessage type reference
