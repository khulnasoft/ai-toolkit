...
```tsx title="app/api/chat/route.ts" {7-25,32-37}
import {
...
```tsx title="app/api/chat/route.ts" {3,10-24}
import {
  convertToModelMessages,
  streamText,
  validateUIMessages,
...
```tsx title="app/api/chat/route.ts" {6,11-17}
import { openai } from '@ai-toolkit/openai';
...
You can control the ID format by providing ID generators using [`createIdGenerator()`](/docs/reference/ai-toolkit-core/create-id-generator):

```tsx title="app/api/chat/route.ts" {7-11}
import { createIdGenerator, streamText } from 'ai-toolkit';
...
```tsx title="app/api/chat/route.ts" {8-18}
import {
  generateId,
  streamText,
...
```tsx title="app/api/chat/route.ts" {2-11,14-18}
import { convertToModelMessages, UIMessage, validateUIMessages } from 'ai-toolkit';
...
```tsx title="app/api/chat/route.ts" {19-21}
import { convertToModelMessages, streamText, UIMessage } from 'ai-toolkit';
import { saveChat } from '@util/chat-store';
...
For more robust handling of disconnects, you may want to add resumability on disconnects. Check out the [Chatbot Resume Streams](/docs/ai-toolkit-ui/chatbot-resume-streams) documentation to learn more.
