...
description: Learn how to handle errors in the AI TOOLKIT UI
---


## Warnings

The AI TOOLKIT shows warnings when something might not work as expected.
...
All warnings start with "AI TOOLKIT Warning:" so you can easily find them. For example:

```
AI TOOLKIT Warning: The feature "temperature" is not supported by this model
```
...
Each AI TOOLKIT UI hook also returns an [error](/docs/reference/ai-toolkit-ui/use-chat#error) object that you can use to render the error in your UI.
...
```tsx title="app/page.tsx" {7,18-25,31}
'use client';
...
```tsx title="app/page.tsx" {17-23,35}
'use client';
...
Errors can be processed by passing an [`onError`](/docs/reference/ai-toolkit-ui/use-chat#on-error) callback function as an option to the [`useChat`](/docs/reference/ai-toolkit-ui/use-chat) or [`useCompletion`](/docs/reference/ai-toolkit-ui/use-completion) hooks.
The callback function receives an error object as an argument.

```tsx title="app/page.tsx" {6-9}
import { useChat } from '@ai-toolkit/react';
