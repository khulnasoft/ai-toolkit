...
The [`useObject`](/docs/reference/ai-toolkit-ui/use-object) hook allows you to create interfaces that represent a structured JSON object that is being streamed.
...
```ts title='app/api/notifications/schema.ts'
import { z } from 'zod';
...
The client uses [`useObject`](/docs/reference/ai-toolkit-ui/use-object) to stream the object generation process.
...
On the server, we use [`streamText`](/docs/reference/ai-toolkit-core/stream-text) with [`Output.object()`](/docs/reference/ai-toolkit-core/output#output-object) to stream the object generation process.

```typescript title='app/api/notifications/route.ts'
...
```tsx title='app/classify/page.tsx'
'use client';
...
```typescript title='app/api/classify/route.ts'
import { streamText, Output } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
```tsx title='app/page.tsx' {6,13-20,24}
'use client';
...
```tsx title='app/page.tsx' {6,14-16}
'use client';
