...
Check out our [chatbot with tools guide](/docs/ai-toolkit-ui/chatbot-with-tool-calling) to learn how to use tools in your chatbot.
...
```tsx title='app/page.tsx' {6,22-29,36}
'use client';
...
```tsx title="app/page.tsx" {6,20-27,33}
'use client';
...
Please also see the [error handling](/docs/ai-toolkit-ui/error-handling) guide for more information.
...
```ts title="app/api/chat/route.ts" {3,4}
export async function POST(req: Request) {
...
For complete examples with type safety and advanced use cases, see the [Message Metadata documentation](/docs/ai-toolkit-ui/message-metadata).
...
To learn more about building custom transports, refer to the [Transport API documentation](/docs/ai-toolkit-ui/transport).
...
For more details, see the [DirectChatTransport reference](/docs/reference/ai-toolkit-ui/direct-chat-transport).
...
```ts title="app/api/chat/route.ts" {13-27}
import { convertToModelMessages, streamText, UIMessage } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
Track token consumption and resource usage with [message metadata](/docs/ai-toolkit-ui/message-metadata):
...
Check out the [stream protocol guide](/docs/ai-toolkit-ui/stream-protocol) for more information.
...
```ts title="app/api/chat/route.ts" {13}
import { convertToModelMessages, streamText, UIMessage } from 'ai-toolkit';
...
<Note>
  ...
  parts](/docs/foundations/prompts#file-parts). You will need to
...
When working with tools in TypeScript, AI TOOLKIT UI provides type inference helpers to ensure type safety for your tool inputs and outputs.
...
```tsx
import { InferUITool } from 'ai-toolkit';
import { z } from 'zod';
...
```tsx
import { InferUITools, ToolSet } from 'ai-toolkit';
import { z } from 'zod';
...
You can use these inferred types to create a custom UIMessage type and pass it to various AI TOOLKIT UI functions:

```tsx
import { InferUITools, UIMessage, UIDataTypes } from 'ai-toolkit';
