...
description: Learn how to build Generative UI with AI TOOLKIT UI.
---
...
At the core of generative UI are [ tools ](/docs/ai-toolkit-core/tools-and-tool-calling), which are functions you provide to the model to perform specialized tasks like getting the weather in a location. The model can decide when and how to use these tools based on the context of the conversation.
...
import { streamText, convertToModelMessages, UIMessage, stepCountIs } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
import { tool as createTool } from 'ai-toolkit';
import { z } from 'zod';
...
```ts title="app/api/chat/route.ts" {3,8,14}
import { streamText, convertToModelMessages, UIMessage, stepCountIs } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
```tsx title="components/weather.tsx"
type WeatherProps = {
  temperature: number;
  weather: string;
  location: string;
...
To check if the model has called a tool, you can check the `parts` array of the UIMessage object for tool-specific parts. In AI TOOLKIT 5.0, tool parts use typed naming: `tool-${toolName}` instead of generic types.

Update your `page.tsx` file:

```tsx title="app/page.tsx" {4,9,14-15,19-46}
'use client';
