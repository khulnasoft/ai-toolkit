---
title: Expo
description: Learn how to build your first agent with the AI TOOLKIT and Expo.
---
...
2. Call [`streamText`](/docs/reference/ai-toolkit-core/stream-text), which is imported from the `ai` package. This function accepts a configuration object that contains a `model` provider (imported from `ai`) and `messages` (defined in step 1). You can pass additional [settings](/docs/ai-toolkit-core/settings) to further customize the model's behavior.
...
```tsx title="app/api/chat+api.ts" {2,11-25}
...
```tsx title="app/(tabs)/index.tsx" {31-35}
import { generateAPIUrl } from '@/utils';
import { useChat } from '@ai-toolkit/react';
import { DefaultChatTransport } from 'ai-toolkit';
import { fetch as expoFetch } from 'expo/fetch';
...
```tsx title="app/api/chat+api.ts" {10}
import {
  streamText,
  UIMessage,
...
```tsx title="app/api/chat+api.ts" {28-41}
import {
  streamText,
  UIMessage,
  convertToModelMessages,
...
```tsx title="app/(tabs)/index.tsx" {37-42}
import { generateAPIUrl } from '@/utils';
import { useChat } from '@ai-toolkit/react';
import { DefaultChatTransport } from 'ai-toolkit';
import { fetch as expoFetch } from 'expo/fetch';
