...
description: Learn how to build your first agent with the AI TOOLKIT and TanStack Start.
---


The AI TOOLKIT is a powerful TypeScript library designed to help developers build AI-powered applications.

In this quickstart tutorial, you'll build a simple agent with a streaming chat user interface. Along the way, you'll learn key concepts and techniques that are fundamental to using the AI TOOLKIT in your own projects.
...
Install `ai` and `@ai-toolkit/react`, the AI package and AI TOOLKIT's React hooks. The AI TOOLKIT's [ Vercel AI Gateway provider ](/providers/ai-toolkit-providers/ai-gateway) ships with the `ai` package. You'll also install `zod`, a schema validation library used for defining tool inputs.
...
<Note>
  ...
  provider or model by installing its package. Check out available [AI TOOLKIT
  providers](/providers/ai-toolkit-providers) for more information.
</Note>

<div className="my-4">
  <Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
    <Tab>
      <Snippet text="pnpm add ai @ai-toolkit/react zod" dark />
    </Tab>
    <Tab>
      <Snippet text="npm install ai @ai-toolkit/react zod" dark />
    </Tab>
    <Tab>
      <Snippet text="yarn add ai @ai-toolkit/react zod" dark />
    </Tab>

    <Tab>
      <Snippet text="bun add ai @ai-toolkit/react zod" dark />
    </Tab>
...
<Note className="mb-4">
  The AI TOOLKIT's Vercel AI Gateway Provider will default to using the
...
```tsx title="src/routes/api/chat.ts"
...
2. Call [`streamText`](/docs/reference/ai-toolkit-core/stream-text), which is imported from the `ai` package. This function accepts a configuration object that contains a `model` provider and `messages` (defined in step 1). You can pass additional [settings](/docs/ai-toolkit-core/settings) to further customize the model's behavior. The `messages` key expects a `ModelMessage[]` array. This type is different from `UIMessage` in that it does not include metadata, such as timestamps or sender information. To convert between these types, we use the `convertToModelMessages` function, which strips the UI-specific metadata and transforms the `UIMessage[]` array into the `ModelMessage[]` format that the model expects.
3. The `streamText` function returns a [`StreamTextResult`](/docs/reference/ai-toolkit-core/stream-text#result-object). This result object contains the [ `toUIMessageStreamResponse` ](/docs/reference/ai-toolkit-core/stream-text#to-data-stream-response) function which converts the result to a streamed response object.
...
The AI TOOLKIT supports dozens of model providers through [first-party](/providers/ai-toolkit-providers), [OpenAI-compatible](/providers/openai-compatible-providers), and [ community ](/providers/community-providers) packages.

This quickstart uses the [Vercel AI Gateway](https://vercel.com/ai-gateway) provider, which is the default [global provider](/docs/ai-toolkit-core/provider-management#global-provider-configuration). This means you can access models using a simple string in the model configuration:
...
```ts
// Option 1: Import from 'ai-toolkit' package (included by default)
...
<div className="my-4">
  <Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
    <Tab>
      <Snippet text="pnpm add @ai-toolkit/openai" dark />
    </Tab>
    <Tab>
      <Snippet text="npm install @ai-toolkit/openai" dark />
    </Tab>
    <Tab>
      <Snippet text="yarn add @ai-toolkit/openai" dark />
    </Tab>

    <Tab>
      <Snippet text="bun add @ai-toolkit/openai" dark />
    </Tab>
...
You can change the default global provider so string model references use your preferred provider everywhere in your application. Learn more about [provider management](/docs/ai-toolkit-core/provider-management#global-provider-configuration).

Pick the approach that best matches how you want to manage providers across your application.

## Wire up the UI

Now that you have a Route Handler that can query an LLM, it's time to setup your frontend. The AI TOOLKIT's [ UI ](/docs/ai-toolkit-ui) package abstracts the complexity of a chat interface into one hook, [`useChat`](/docs/reference/ai-toolkit-ui/use-chat).
...
Head to your browser and open http://localhost:3000. You should see an input field. Test it out by entering a message and see the AI chatbot respond in real-time! The AI TOOLKIT makes it fast and easy to build AI chat interfaces with TanStack Start.
...
```tsx title="src/routes/api/chat.ts" {2,13-27}
import { streamText, UIMessage, convertToModelMessages, tool } from 'ai-toolkit';
__PROVIDER_IMPORT__;
import { createFileRoute } from '@tanstack/react-router';
import { z } from 'zod';
...
```tsx title="src/routes/index.tsx" {16-21}
import { createFileRoute } from '@tanstack/react-router';
import { useChat } from '@ai-toolkit/react';
import { useState } from 'react';
...
```tsx title="src/routes/api/chat.ts" {34-47}
import {
  streamText,
  UIMessage,
  convertToModelMessages,
...
```tsx title="src/routes/index.tsx" {21}
import { createFileRoute } from '@tanstack/react-router';
import { useChat } from '@ai-toolkit/react';
import { useState } from 'react';
