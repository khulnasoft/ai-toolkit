---
title: Vue.js (Nuxt)
description: Learn how to build your first agent with the AI TOOLKIT and Vue.js (Nuxt).
---
...
Install `ai` and `@ai-toolkit/vue`. The Vercel AI Gateway provider ships with the `ai` package.
...
<div className="my-4">
  <Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
    <Tab>
      <Snippet text="pnpm add ai @ai-toolkit/vue zod" dark />
    </Tab>
    <Tab>
      <Snippet text="npm install ai @ai-toolkit/vue zod" dark />
    </Tab>
    <Tab>
      <Snippet text="yarn add ai @ai-toolkit/vue zod" dark />
    </Tab>

    <Tab>
      <Snippet text="bun add ai @ai-toolkit/vue zod" dark />
    </Tab>
...
```typescript title="server/api/chat.ts"
import {
  streamText,
  UIMessage,
...
4. The `streamText` function returns a [`StreamTextResult`](/docs/reference/ai-toolkit-core/stream-text#result). This result object contains the [ `toUIMessageStreamResponse` ](/docs/reference/ai-toolkit-core/stream-text#to-ui-message-stream-response) function which converts the result to a streamed response object.
...
Now that you have an API route that can query an LLM, it's time to setup your frontend. The AI TOOLKIT's [ UI ](/docs/ai-toolkit-ui/overview) package abstract the complexity of a chat interface into one hook, [`useChat`](/docs/reference/ai-toolkit-ui/use-chat).
...
Head to your browser and open http://localhost:3000. You should see an input field. Test it out by entering a message and see the AI chatbot respond in real-time! The AI TOOLKIT makes it fast and easy to build AI chat interfaces with Nuxt.
...
```typescript title="server/api/chat.ts" {1,16-32}
import {
  createGateway,
  streamText,
  UIMessage,
...
```typescript title="pages/index.vue" {16-18}
<script setup lang="ts">
import { Chat } from "@ai-toolkit/vue";
import { ref } from "vue";
...
```typescript title="server/api/chat.ts" {22}
import {
  createGateway,
  streamText,
  UIMessage,
...
```typescript title="server/api/chat.ts" {32-45}
import {
  createGateway,
  streamText,
  UIMessage,
...
```typescript title="pages/index.vue" {24}
<script setup lang="ts">
import { Chat } from "@ai-toolkit/vue";
import { ref } from "vue";
