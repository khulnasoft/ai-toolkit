---
title: Node.js
description: Learn how to build your first agent with the AI TOOLKIT and Node.js.
---
...
Install `ai`, the AI TOOLKIT, along with other necessary dependencies.
...
<Note className="mb-4">
  The AI TOOLKIT will use the `AI_GATEWAY_API_KEY` environment variable to
...
- Call [`streamText`](/docs/reference/ai-toolkit-core/stream-text), which is imported from the `ai` package. This function accepts a configuration object that contains a `model` provider and `messages`.
...
You should see a prompt in your terminal. Test it out by entering a message and see the AI agent respond in real-time! The AI TOOLKIT makes it fast and easy to build AI chat interfaces with Node.js.
...
```ts title="index.ts" {38-41}
import { ModelMessage, streamText, tool, stepCountIs } from 'ai-toolkit';
__PROVIDER_IMPORT__;
import 'dotenv/config';
import { z } from 'zod';
import * as readline from 'node:readline/promises';
...
You've built an AI agent using the AI TOOLKIT! From here, you have several paths to explore:
