---
title: Svelte
description: Learn how to build your first agent with the AI TOOLKIT and Svelte.
---
...
Install `ai` and `@ai-toolkit/svelte`, the AI package and AI TOOLKIT's Svelte bindings. The AI TOOLKIT's [ Vercel AI Gateway provider ](/providers/ai-toolkit-providers/ai-gateway) ships with the `ai` package. You'll also install `zod`, a schema validation library used for defining tool inputs.
...
<div className="my-4">
  <Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
    <Tab>
      <Snippet text="pnpm add -D ai @ai-toolkit/svelte zod" dark />
    </Tab>
    <Tab>
      <Snippet text="npm install -D ai @ai-toolkit/svelte zod" dark />
    </Tab>
    <Tab>
      <Snippet text="yarn add -D ai @ai-toolkit/svelte zod" dark />
    </Tab>
    <Tab>
      <Snippet text="bun add -d ai @ai-toolkit/svelte zod" dark />
    </Tab>
  </Tabs>
</div>
...
```tsx title="src/routes/api/chat/+server.ts"
import {
  streamText,
  type UIMessage,
...
3. Call [`streamText`](/docs/reference/ai-toolkit-core/stream-text), which is imported from the `ai` package. This function accepts a configuration object that contains a `model` provider (defined in step 1) and `messages` (defined in step 2). You can pass additional [settings](/docs/ai-toolkit-core/settings) to further customize the model's behavior. The `messages` key expects a `ModelMessage[]` array. This type is different from `UIMessage` in that it does not include metadata, such as timestamps or sender information. To convert between these types, we use the `convertToModelMessages` function, which strips the UI-specific metadata and transforms the `UIMessage[]` array into the `ModelMessage[]` format that the model expects.
...
Now that you have an API route that can query an LLM, it's time to set up your frontend. The AI TOOLKIT's [UI](/docs/ai-toolkit-ui) package abstracts the complexity of a chat interface into one class, `Chat`.
Its properties and API are largely the same as React's [`useChat`](/docs/reference/ai-toolkit-ui/use-chat).
...
```tsx title="src/routes/api/chat/+server.ts" {2,3,17-31}
import {
  createGateway,
  streamText,
  type UIMessage,
...
```ts title="src/routes/api/chat/+server.ts" {15}
import {
  createGateway,
  streamText,
  type UIMessage,
...
```tsx title="src/routes/api/chat/+server.ts" {32-45}
import {
  createGateway,
  streamText,
  type UIMessage,
...
```svelte title="src/routes/+page.svelte" {17}
<script lang="ts">
  import { Chat } from '@ai-toolkit/svelte';
...
## How does `@ai-toolkit/svelte` differ from `@ai-toolkit/react`?
