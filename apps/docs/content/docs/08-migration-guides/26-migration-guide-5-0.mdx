---
title: Migrate AI TOOLKIT 4.x to 5.0
description: Learn how to upgrade AI TOOLKIT 4.x to 5.0.
---

# Migrate AI TOOLKIT 4.x to 5.0

## Recommended Migration Process

1. Backup your project. If you use a versioning control system, make sure all previous versions are committed.
1. Upgrade to AI TOOLKIT 5.0.
1. Automatically migrate your code using one of these approaches:
   - Use the [AI TOOLKIT 5 Migration MCP Server](#ai-toolkit-5-migration-mcp-server) for AI-assisted migration in Cursor or other MCP-compatible coding agents
...
## AI TOOLKIT 5 Migration MCP Server

The [AI TOOLKIT 5 Migration Model Context Protocol (MCP) Server](https://github.com/vercel-labs/ai-toolkit-5-migration-mcp-server) provides an automated way to migrate your project using a coding agent. This server has been designed for Cursor, but should work with any coding agent that supports MCP.
...
{
  "mcpServers": {
    "ai-toolkit-5-migration": {
      "url": "https://ai-toolkit-5-migration-mcp-server.vercel.app/api/mcp"
    }
  }
}
```
...
```
Please migrate this project to AI TOOLKIT 5 using the ai-toolkit-5-migration mcp server. Start by creating a checklist.
```

For more information, see the [AI TOOLKIT 5 Migration MCP Server repository](https://github.com/vercel-labs/ai-toolkit-5-migration-mcp-server).

## AI TOOLKIT 5.0 Package Versions

You need to update the following packages to the following versions in your `package.json` file(s):

- `ai` package: `5.0.0`
- `@ai-toolkit/provider` package: `2.0.0`
- `@ai-toolkit/provider-utils` package: `3.0.0`
- `@ai-toolkit/*` packages: `2.0.0` (other `@ai-toolkit` packages)
...
```
npm install ai @ai-toolkit/react @ai-toolkit/openai zod@^4.1.8
```
...
The AI TOOLKIT provides Codemod transformations to help upgrade your codebase when a
...
```sh
npx @ai-toolkit/codemod v5/rename-format-stream-part src/
```

See also the [table of codemods](#codemod-table). In addition, the latest set of
codemods can be found in the
[`@ai-toolkit/codemod`](https://github.com/khulnasoft/ai-toolkit/tree/main/packages/special/codemod/src/codemods)
repository.

## AI TOOLKIT Core Changes
...
```tsx filename="AI TOOLKIT 4.0"
import { Message, CreateMessage } from 'ai-toolkit';
```

```tsx filename="AI TOOLKIT 5.0"
import { UIMessage, CreateUIMessage } from 'ai-toolkit';
```

##### `convertToCoreMessages` → `convertToModelMessages`

```tsx filename="AI TOOLKIT 4.0"
import { convertToCoreMessages, streamText } from 'ai-toolkit';
...
<Note>
  ...
  reference](/docs/reference/ai-toolkit-core/model-message).
</Note>
...
```tsx filename="AI TOOLKIT 4.0"
import { type Message } from 'ai-toolkit'; // v4 Message type
...
```tsx filename="AI TOOLKIT 5.0"
import { type UIMessage, type ModelMessage } from 'ai-toolkit';
...
```tsx filename="AI TOOLKIT 5.0"
import { createUIMessageStream, createUIMessageStreamResponse } from 'ai-toolkit';
...
<Note>
  ...
  [Streaming Data guide](/docs/ai-toolkit-ui/streaming-data).
</Note>
...
```tsx filename="AI TOOLKIT 4.0"
import { ToolExecutionError } from 'ai-toolkit';
...
The `toolCallStreaming` option has been removed in AI TOOLKIT 5.0. Tool call streaming is now always enabled by default.
...
AI TOOLKIT 5.0 introduces dynamic tools for handling tools with unknown types at development time, such as MCP tools without schemas or user-defined functions at runtime.
...
```tsx filename="AI TOOLKIT 5.0"
import { dynamicTool } from 'ai-toolkit';
import { z } from 'zod';
...
```tsx filename="AI TOOLKIT 5.0"
import { MCPClient } from 'ai-toolkit';
...
```tsx filename="AI TOOLKIT 5.0"
import { isToolUIPart, getToolName } from 'ai-toolkit';
...
```tsx filename="AI TOOLKIT 5.0"
import { stepCountIs, hasToolCall } from 'ai-toolkit';
...
## AI TOOLKIT UI Changes

### Package Structure Changes

### `@ai-toolkit/rsc` Package Extraction
...
<Note>
  Don't forget to install the new package: `npm install @ai-toolkit/rsc`
</Note>

### React UI Hooks Moved to `@ai-toolkit/react`

The deprecated `ai/react` export has been removed in favor of `@ai-toolkit/react`.
...
<Note>
  Don't forget to install the new package: `npm install @ai-toolkit/react`
</Note>
...
import { streamText, convertToModelMessages, stepCountIs } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
```tsx filename="AI TOOLKIT 4.0"
import { useChat, type Message } from '@ai-toolkit/react';
...
```tsx filename="AI TOOLKIT 5.0"
import { useChat, type UIMessage } from '@ai-toolkit/react';
...
```tsx filename="AI TOOLKIT 4.0"
import type { RequestOptions } from 'ai-toolkit';
```

```tsx filename="AI TOOLKIT 5.0"
import type { CompletionRequestOptions } from 'ai-toolkit';
```
...
For more details on request configuration, see the [Chatbot guide](/docs/ai-toolkit-ui/chatbot#request-configuration).
...
```tsx filename="AI TOOLKIT 5.0 - Client"
'use client';
...
```tsx filename="AI TOOLKIT 5.0 - onFinish"
'use client';
...
```typescript filename="@ai-toolkit/vue v1"
<script setup>
...
```typescript filename="@ai-toolkit/vue v2"
<script setup>
...
```js filename="@ai-toolkit/svelte v1"
...
```js filename="@ai-toolkit/svelte v2"
import { Chat } from '@ai-toolkit/svelte';
import { DefaultChatTransport } from 'ai-toolkit';
...
#### `@ai-toolkit/ui-utils` Package Removal

The `@ai-toolkit/ui-utils` package has been removed and its exports moved to the main `ai` package.
...
```tsx filename="AI TOOLKIT 5.0"
import { getTextFromDataUrl } from 'ai-toolkit';
```
...
```tsx filename="AI TOOLKIT 4.0"
import { useAssistant } from '@ai-toolkit/react';
```
...
For an implementation of the assistant functionality with AI TOOLKIT v5, see this [example repository](https://github.com/vercel-labs/ai-toolkit-openai-assistants-api).
...
#### LangChain Adapter Moved to `@ai-toolkit/langchain`

The `LangChainAdapter` has been moved to `@ai-toolkit/langchain` and the API has been updated to use UI message streams.
...
<Note>
  Don't forget to install the new package: `npm install @ai-toolkit/langchain`
</Note>

#### LlamaIndex Adapter Moved to `@ai-toolkit/llamaindex`

The `LlamaIndexAdapter` has been extracted to a separate package `@ai-toolkit/llamaindex` and follows the same UI message stream pattern.

```tsx filename="AI TOOLKIT 4.0"
import { LlamaIndexAdapter } from 'ai-toolkit';

const response = LlamaIndexAdapter.toDataStreamResponse(stream);
```

```tsx filename="AI TOOLKIT 5.0"
import { toUIMessageStream } from '@ai-toolkit/llamaindex';
import { createUIMessageStreamResponse } from 'ai-toolkit';
...
<Note>
  Don't forget to install the new package: `npm install @ai-toolkit/llamaindex`
</Note>
...
```tsx filename="AI TOOLKIT 4.0"
import { createDataStream, formatDataStreamPart } from 'ai-toolkit';
...
```tsx filename="AI TOOLKIT 5.0"
import { createUIMessageStream } from 'ai-toolkit';
...
```tsx filename="AI TOOLKIT 4.0"
import { DataStreamToSSETransformStream } from 'ai-toolkit';
```

```tsx filename="AI TOOLKIT 5.0"
import { JsonToSseTransformStream } from 'ai-toolkit';
```
...
```tsx filename="AI TOOLKIT 4.0"
import { IDGenerator } from 'ai-toolkit';
```
...
`LanguageModelV3` must now be imported from `@ai-toolkit/provider`.

```tsx filename="AI TOOLKIT 4.0"
import { LanguageModelV3 } from 'ai-toolkit';
```
...
```tsx filename="AI TOOLKIT 4.0"
import { LanguageModelV1Middleware } from 'ai-toolkit';
```
...
```tsx filename="AI TOOLKIT 4.0"
import { experimental_wrapLanguageModel } from 'ai-toolkit';
```
...
```tsx filename="AI TOOLKIT 5.0"
import { streamText, convertToModelMessages, UIMessage } from 'ai-toolkit';
...
For more details on message IDs and persistence, see the [Chatbot Message Persistence guide](/docs/ai-toolkit-ui/chatbot-message-persistence#message-ids).
...
```tsx filename="AI TOOLKIT 5.0 - Advanced"
import {
  createUIMessageStream,
  createUIMessageStreamResponse,
  streamText,
...
In AI TOOLKIT 5, the default OpenAI provider instance uses the Responses API, while AI TOOLKIT 4 used the Chat Completions API. The Chat Completions API remains fully supported and you can use it with `openai.chat(...)`.
...
In AI TOOLKIT 4.0, you could set the `strictSchemas` option on Responses models (which defaulted to `true`). This option has been renamed to `strictJsonSchema` in AI TOOLKIT 5.0 and now defaults to `false`.
...
const result = await generateObject({
  ...
  providerOptions: {
    openai: {
      strictSchemas: true, // default behavior in AI TOOLKIT 4
...
const result = await generateObject({
  ...
  providerOptions: {
    openai: {
      strictJsonSchema: true, // defaults to false, opt back in to the AI TOOLKIT 4 strict behavior
    } satisfies OpenAIResponsesProviderOptions,
  },
});
```

If you call `openai.chat(...)` to use the Chat Completions API directly, you can type it with `OpenAIChatLanguageModelOptions`. AI TOOLKIT 5 adds the same `strictJsonSchema` option there as well.
...
```tsx filename="AI TOOLKIT 5.0 (Chat Completions API)"
import { z } from 'zod';
import { generateObject } from 'ai-toolkit';
import {
  openai,
...
```tsx filename="AI TOOLKIT 5.0"
import { simulateStreamingMiddleware, wrapLanguageModel } from 'ai-toolkit';
...
If you experience TypeScript server crashes, slow type checking, or errors like "Type instantiation is excessively deep and possibly infinite" when using Zod with AI TOOLKIT 5.0:
...
The following table lists available codemods for the AI TOOLKIT 5.0 upgrade
process.
...
| Move React to AI TOOLKIT                         | `v5/move-react-to-ai-toolkit`                         |
...
This section documents breaking changes between different beta versions of AI TOOLKIT 5.0. If you're upgrading from an earlier v5 beta version to a later one, check this section for any changes that might affect your code.
...
```tsx filename="AI TOOLKIT 5.0 (before beta.26)"
for await (const chunk of result.fullStream) {
  switch (chunk.type) {
    case 'text-delta': {
...
```tsx filename="AI TOOLKIT 5.0 (beta.26 and later)"
for await (const chunk of result.fullStream) {
  switch (chunk.type) {
    case 'text-delta': {
      process.stdout.write(chunk.text);
