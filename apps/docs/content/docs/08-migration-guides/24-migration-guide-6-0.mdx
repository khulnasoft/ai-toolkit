---
title: Migrate AI TOOLKIT 5.x to 6.0
description: Learn how to upgrade AI TOOLKIT 5.x to 6.0.
---

# Migrate AI TOOLKIT 5.x to 6.0

## Recommended Migration Process

1. Backup your project. If you use a versioning control system, make sure all previous versions are committed.
1. Upgrade to AI TOOLKIT 6.0.
1. Follow the breaking changes guide below.
1. Verify your project is working as expected.
1. Commit your changes.

## AI TOOLKIT 6.0 Package Versions

You need to update the following packages to the latest versions in your `package.json` file(s):

- `ai` package: `^6.0.0`
- `@ai-toolkit/provider` package: `^3.0.0`
- `@ai-toolkit/provider-utils` package: `^4.0.0`
- `@ai-toolkit/*` packages: `^3.0.0`

An example upgrade command would be:

```
pnpm install ai@latest @ai-toolkit/react@latest @ai-toolkit/openai@latest
```
...
<Note>
  There is also an `npx @ai-toolkit/codemod upgrade` command, but it runs all
...
```sh
npx @ai-toolkit/codemod v6/rename-text-embedding-to-embedding src/
```
...
| `add-await-converttomodelmessages`                       | Adds `await` to `convertToModelMessages` calls (now async in AI TOOLKIT 6)                         |
...
```tsx filename="AI TOOLKIT 5"
import { Experimental_Agent as Agent, stepCountIs } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
The deprecated `CoreMessage` type and related functions have been removed ([PR #10710](https://github.com/khulnasoft/ai-toolkit/pull/10710)). Replace `convertToCoreMessages` with `convertToModelMessages`.

```tsx filename="AI TOOLKIT 5"
import { convertToCoreMessages, type CoreMessage } from 'ai-toolkit';

const coreMessages = convertToCoreMessages(messages); // CoreMessage[]
```

```tsx filename="AI TOOLKIT 6"
import { convertToModelMessages, type ModelMessage } from 'ai-toolkit';
...
`generateObject` and `streamObject` have been deprecated ([PR #10754](https://github.com/khulnasoft/ai-toolkit/pull/10754)).
...
```tsx filename="AI TOOLKIT 6"
import { streamText, Output } from 'ai-toolkit';
__PROVIDER_IMPORT__;
import { z } from 'zod';
...
Learn more about [generating structured data](/docs/ai-toolkit-core/generating-structured-data).

### async `convertToModelMessages`

`convertToModelMessages()` is async in AI TOOLKIT 6 to support async `Tool.toModelOutput()`.

```tsx filename="AI TOOLKIT 5"
import { convertToModelMessages } from 'ai-toolkit';
...
`toModelOutput()` receives a parameter object with an `output` property in AI TOOLKIT 6.

In AI TOOLKIT 5, the `output` was the arguments.
...
Strict mode for tools is now controlled by setting `strict` on each tool ([PR #10817](https://github.com/khulnasoft/ai-toolkit/pull/10817)). This enables fine-grained control over strict tool calls, which is important since strict mode depends on the specific tool input schema.
...
AI TOOLKIT 6 introduces more flexible tool output and result content support ([PR #9605](https://github.com/khulnasoft/ai-toolkit/pull/9605)), enabling richer tool interactions and better support for complex tool execution patterns.

### `ToolCallRepairFunction` Signature

The `system` parameter in the `ToolCallRepairFunction` type now accepts `SystemModelMessage` in addition to `string` ([PR #10635](https://github.com/khulnasoft/ai-toolkit/pull/10635)). This allows for more flexible system message configuration, including provider-specific options like caching.

```tsx filename="AI TOOLKIT 5"
import type { ToolCallRepairFunction } from 'ai-toolkit';
...
```tsx filename="AI TOOLKIT 6"
import type { ToolCallRepairFunction, SystemModelMessage } from 'ai-toolkit';
...
The `textEmbeddingModel` and `textEmbedding` methods on providers have been renamed to `embeddingModel` and `embedding` respectively. Additionally, generics have been removed from `EmbeddingModel`, `embed`, and `embedMany` ([PR #10592](https://github.com/khulnasoft/ai-toolkit/pull/10592)).
...
AI TOOLKIT 6 introduces a warning logger that outputs deprecation warnings and best practice recommendations ([PR #8343](https://github.com/khulnasoft/ai-toolkit/pull/8343)).

To disable warning logging, set the `AI_TOOLKIT_LOG_WARNINGS` environment variable to `false`:

```bash
export AI_TOOLKIT_LOG_WARNINGS=false
```

### Warning Type Unification

Separate warning types for each generation function have been consolidated into a single `Warning` type exported from the `ai` package ([PR #10631](https://github.com/khulnasoft/ai-toolkit/pull/10631)).
...
import type { Warning } from 'ai-toolkit';
```
...
The tool UI part helper functions have been renamed to better reflect their purpose and to accommodate both static and dynamic tool parts ([PR #XXXX](https://github.com/khulnasoft/ai-toolkit/pull/XXXX)).
...
```tsx filename="AI TOOLKIT 5"
import { isToolUIPart } from 'ai-toolkit';
...
```tsx filename="AI TOOLKIT 5"
import { isToolOrDynamicToolUIPart } from 'ai-toolkit';
...
```tsx filename="AI TOOLKIT 5"
import { getToolName } from 'ai-toolkit';
...
```tsx filename="AI TOOLKIT 6"
import { getStaticToolName } from 'ai-toolkit';
...
```tsx filename="AI TOOLKIT 5"
import { getToolOrDynamicToolName } from 'ai-toolkit';
...
The `strictJsonSchema` setting for JSON outputs and tool calls is enabled by default ([PR #10752](https://github.com/khulnasoft/ai-toolkit/pull/10752)). This improves stability and ensures valid JSON output that matches your schema.
...
The `structuredOutputs` provider option has been removed from chat models ([PR #10752](https://github.com/khulnasoft/ai-toolkit/pull/10752)). Use `strictJsonSchema` instead.

### Azure

#### Default Provider Uses Responses API

The `@ai-toolkit/azure` provider now uses the Responses API by default when calling `azure()` ([PR #9868](https://github.com/khulnasoft/ai-toolkit/pull/9868)). To use the previous Chat Completions API behavior, use `azure.chat()` instead.
...
For the **Responses API**, the `@ai-toolkit/azure` provider now uses `azure` as the key for `providerMetadata` and `providerOptions` instead of `openai`. The `openai` key is still supported for `providerOptions` input, but resulting `providerMetadata` output now uses `azure`.
...
const result = await generateText({
  ...
  providerOptions: {
    openai: {
      // AI TOOLKIT 5: use `openai` key for Responses API options
...
const result = await generateText({
  ...
  providerOptions: {
    azure: {
      // AI TOOLKIT 6: use `azure` key for Responses API options
      reasoningSummary: 'auto',
    },
  },
});
...
Anthropic has [ introduced native structured outputs for Claude Sonnet 4.5 and later models ](https://www.claude.com/blog/structured-outputs-on-the-claude-developer-platform). The `@ai-toolkit/anthropic` provider now includes a `structuredOutputMode` option to control how structured outputs are generated ([PR #10502](https://github.com/khulnasoft/ai-toolkit/pull/10502)).
...
The `@ai-toolkit/google-vertex` provider now uses `vertex` as the key for `providerMetadata` and `providerOptions` instead of `google`. The `google` key is still supported for `providerOptions` input, but resulting `providerMetadata` output now uses `vertex`.
