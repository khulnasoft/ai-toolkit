---
title: RAG Chatbot
description: Learn how to build a RAG Chatbot with the AI TOOLKIT and Next.js
---
...
- [ AI TOOLKIT ](https://sdk.khulnasoft.com/docs)
...
To reduce the scope of this guide, you will be starting with a [repository](https://github.com/khulnasoft/ai-toolkit-toolkit-rag-starter) that already has a few things set up for you:
...
<Snippet
  text={[
    'git clone https://github.com/khulnasoft/ai-toolkit-toolkit-rag-starter',
...
- Create a free Postgres database with [Khulnasoft Postgres](https://khulnasoft.com/docs/storage/vercel-postgres); or
...
<Note>
  ...
  info](https://github.com/khulnasoft/ai-toolkit-toolkit-rag-starter/issues/1).
</Note>
...
<Snippet text="pnpm add ai @ai-toolkit/react @ai-toolkit/openai" />

This will install the [AI TOOLKIT](https://sdk.khulnasoft.com/docs), AI TOOLKIT's React hooks, and AI TOOLKIT's [OpenAI provider](/providers/ai-toolkit-providers/openai).
...
Great! Let's build the frontend. The AI TOOLKIT’s [`useChat`](/docs/reference/ai-toolkit-ui/use-chat) hook allows you to easily create a conversational user interface for your chatbot application.
...
In this code, you declare and export an asynchronous function called POST. You retrieve the `messages` from the request body and then pass them to the [`streamText`](/docs/reference/ai-toolkit-core/stream-text) function imported from the AI TOOLKIT, alongside the model you would like to use. Finally, you return the model’s response in `AIStreamResponse` format.
...
- **parameters**: [Zod schema](https://sdk.khulnasoft.com/docs/foundations/tools#schema-specification-and-validation-with-zod) that defines the parameters necessary for the tool to run.
- **execute**: An asynchronous function that is called with the arguments from the tool call.

In simple terms, on each generation, the model will decide whether it should call the tool. If it deems it should call the tool, it will extract the parameters from the input and then append a new `message` to the `messages` array of type `tool-call`. The AI TOOLKIT will then run the `execute` function with the parameters provided by the `tool-call` message.
...
The AI TOOLKIT has a feature called [`maxSteps`](/docs/ai-toolkit-core/tools-and-tool-calling#multi-step-calls) which will automatically send tool call results back to the model!
