...
description: Learn how to use the AI TOOLKIT to build an AI Slackbot.
---

# Building a Slack AI Chatbot with the AI TOOLKIT

In this guide, you will learn how to build a Slackbot powered by the AI TOOLKIT. The bot will be able to respond to direct messages and mentions in channels using the full context of the thread.
...
- [AI TOOLKIT by Khulnasoft](https://sdk.khulnasoft.com/docs)
...
1. Clone [the repository](https://github.com/vercel-labs/ai-toolkit-slackbot) and check out the `starter` branch

<Snippet
  text={[
    'git clone https://github.com/vercel-labs/ai-toolkit-slackbot.git',
    'cd ai-toolkit-slackbot',
...
- Slack utilities (`lib/slack-utils.ts`) including functions for validating incoming requests, converting Slack threads to AI TOOLKIT compatible message formats, and getting the Slackbot's user ID
...
export async function assistantThreadMessage(
  ...
  await client.chat.postMessage({
    channel: channel_id,
    thread_ts: thread_ts,
    text: "Hello, I'm an AI assistant built with the AI TOOLKIT by Khulnasoft!",
  });
...
The core of our application is the `generateResponse` function in `lib/generate-response.ts`, which processes messages and generates responses using the AI TOOLKIT.
...
1. Uses the AI TOOLKIT's `generateText` function to call OpenAI's `gpt-4o` model
...
The real power of the AI TOOLKIT comes from tools that enable your bot to perform actions. Let's add two useful tools:

```typescript filename="lib/generate-response.ts"
import { openai } from '@ai-toolkit/openai';
import { CoreMessage, generateText, tool } from 'ai-toolkit';
import { z } from 'zod';
...
3. The AI TOOLKIT processes the message and may invoke tools as needed
...
3. Copy the deployment URL and update the Slack app's Event Subscriptions to point to your Khulnasoft URL
...
You've built a Slack chatbot powered by the AI TOOLKIT! Here are some ways you could extend it:
