---
title: AI TOOLKIT UI
description: Reference documentation for the AI TOOLKIT UI
collapsed: true
---

# AI TOOLKIT UI

[AI TOOLKIT UI](/docs/ai-toolkit-ui) is designed to help you build interactive chat, completion, and assistant applications with ease.
It is framework-agnostic toolkit, streamlining the integration of advanced AI functionalities into your applications.

AI TOOLKIT UI contains the following hooks:

<IndexCards
  cards={[
    {
      title: 'useChat',
      description:
        'Use a hook to interact with language models in a chat interface.',
      href: '/docs/reference/ai-toolkit-ui/use-chat',
    },
    {
      title: 'useCompletion',
      description:
        'Use a hook to interact with language models in a completion interface.',
      href: '/docs/reference/ai-toolkit-ui/use-completion',
    },
    {
      title: 'useObject',
      description: 'Use a hook for consuming a streamed JSON objects.',
      href: '/docs/reference/ai-toolkit-ui/use-object',
    },
    {
      title: 'useAssistant',
      description: 'Use a hook to interact with OpenAI assistants.',
      href: '/docs/reference/ai-toolkit-ui/use-assistant',
    },
    {
      title: 'convertToCoreMessages',
      description:
        'Convert useChat messages to CoreMessages for AI core functions.',
      href: '/docs/reference/ai-toolkit-ui/convert-to-core-messages',
    },
    {
      title: 'appendResponseMessages',
      description:
        'Append CoreMessage[] from an AI response to an existing array of UI messages.',
      href: '/docs/reference/ai-toolkit-ui/append-response-messages',
    },
    {
      title: 'appendClientMessage',
      description:
        'Append a client message to an existing array of UI messages.',
      href: '/docs/reference/ai-toolkit-ui/append-client-message',
    },
    {
      title: 'createDataStream',
      description:
        'Create a data stream to stream additional data to the client.',
      href: '/docs/reference/ai-toolkit-ui/create-data-stream',
    },
    {
      title: 'createDataStreamResponse',
      description:
        'Create a response object to stream additional data to the client.',
      href: '/docs/reference/ai-toolkit-ui/create-data-stream-response',
    },
    {
      title: 'pipeDataStreamToResponse',
      description: 'Pipe a data stream to a Node.js ServerResponse object.',
      href: '/docs/reference/ai-toolkit-ui/pipe-data-stream-to-response',
    },
    {
      title: 'streamData',
      description:
        'Stream additional data to the client along with generations.',
      href: '/docs/reference/ai-toolkit-ui/stream-data',
    },
  ]}
/>

It also contains the following helper functions:

<IndexCards
  cards={[
    {
      title: 'AssistantResponse',
      description: 'Streaming helper for assistant responses.',
      href: '/docs/reference/ai-toolkit-ui/assistant-response',
    },
  ]}
/>

## UI Framework Support

AI TOOLKIT UI supports the following frameworks: [React](https://react.dev/), [Svelte](https://svelte.dev/), [Vue.js](https://vuejs.org/), and [SolidJS](https://www.solidjs.com/).
Here is a comparison of the supported functions across these frameworks:

| Function                                                      | React               | Svelte              | Vue.js              | SolidJS             |
| ------------------------------------------------------------- | ------------------- | ------------------- | ------------------- | ------------------- |
| [useChat](/docs/reference/ai-toolkit-ui/use-chat)             | <Check size={18} /> | <Check size={18} /> | <Check size={18} /> | <Check size={18} /> |
| [useCompletion](/docs/reference/ai-toolkit-ui/use-completion) | <Check size={18} /> | <Check size={18} /> | <Check size={18} /> | <Check size={18} /> |
| [useObject](/docs/reference/ai-toolkit-ui/use-object)         | <Check size={18} /> | <Cross size={18} /> | <Cross size={18} /> | <Check size={18} /> |
| [useAssistant](/docs/reference/ai-toolkit-ui/use-assistant)   | <Check size={18} /> | <Check size={18} /> | <Check size={18} /> | <Check size={18} /> |

<Note>
  [Contributions](https://github.com/khulnasoft/ai-toolkit/blob/main/CONTRIBUTING.md)
  are welcome to implement missing features for non-React frameworks.
</Note>
