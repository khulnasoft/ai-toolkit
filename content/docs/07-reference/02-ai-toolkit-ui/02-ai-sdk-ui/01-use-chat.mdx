...
<Note>
  The `useChat` API has been significantly updated in AI TOOLKIT 5.0. It now uses a
...
<Tabs items={['React', 'Svelte', 'Vue', 'Angular']}>
  ...
  <Tab>
    <Snippet text="import { Chat } from '@ai-toolkit/svelte'" dark prompt={false} />
  </Tab>
  <Tab>
    <Snippet text="import { Chat } from '@ai-toolkit/vue'" dark prompt={false} />
  </Tab>
  <Tab>
    <Snippet
      text="import { Chat } from '@ai-toolkit/angular'"
      dark
      prompt={false}
    />
  </Tab>
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      isOptional: true,
      description:
        'A function to generate unique IDs for messages and the chat. If not provided, the default AI TOOLKIT generateId is used.',
    },
    {
      name: 'onToolCall',
...
- [Chatbot with Tools](/docs/ai-toolkit-ui/chatbot-tool-usage)
