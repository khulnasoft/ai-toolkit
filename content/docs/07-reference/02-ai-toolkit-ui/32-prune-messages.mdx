...
import { pruneMessages, streamText } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
<Snippet text={`import { pruneMessages } from "ai-toolkit"`} prompt={false} />
...
An array of [`ModelMessage`](/docs/reference/ai-toolkit-core/model-message) objects, pruned according to the provided options.
...
```ts
import { pruneMessages } from 'ai-toolkit';
...
For advanced usage and the full list of possible message parts, see [`ModelMessage`](/docs/reference/ai-toolkit-core/model-message) and [`pruneMessages` implementation](https://github.com/khulnasoft/ai-toolkit/blob/main/packages/core/ai/src/generate-text/prune-messages.ts).
