...
`jsonSchema` is a helper function that creates a JSON schema object that is compatible with the AI TOOLKIT.
...
```ts
import { jsonSchema } from 'ai-toolkit';
...
<Snippet text={`import { jsonSchema } from "ai-toolkit"`} prompt={false} />
...
A JSON schema object that is compatible with the AI TOOLKIT.
