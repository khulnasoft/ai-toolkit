...
`safeValidateUIMessages` is an async function that validates UI messages like [`validateUIMessages`](https://studio.khulnasoft.com/docs/reference/ai-toolkit-core/validate-ui-messages), but instead of throwing it returns an object with a `success` key and either `data` or `error`.
...
```typescript
import { safeValidateUIMessages } from 'ai-toolkit';
...
```typescript
import { safeValidateUIMessages, tool } from 'ai-toolkit';
import { z } from 'zod';
