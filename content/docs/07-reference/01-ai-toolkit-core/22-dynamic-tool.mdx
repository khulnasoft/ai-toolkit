...
<Snippet text={`import { dynamicTool } from "ai-toolkit"`} prompt={false} />
...
A `Tool<unknown, unknown>` with `type: 'dynamic'` that can be used with `generateText`, `streamText`, and other AI TOOLKIT functions.
