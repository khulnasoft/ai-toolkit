...
```ts
import { generateText, hasToolCall } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
<Snippet text={`import { hasToolCall } from "ai-toolkit"`} prompt={false} />
...
```ts
import { generateText, hasToolCall, stepCountIs } from 'ai-toolkit';
...
- [`stepCountIs()`](/docs/reference/ai-toolkit-core/step-count-is)
