...
The `Output` object provides output specifications for structured data generation with [`generateText`](/docs/reference/ai-toolkit-core/generate-text) and [`streamText`](/docs/reference/ai-toolkit-core/stream-text). It allows you to specify the expected shape of the generated data and handles validation automatically.
...
<Snippet text={`import { Output } from "ai-toolkit"`} prompt={false} />
...
<Note>
  With `Output.json()`, the AI TOOLKIT only checks that the response is valid
  JSON; it doesn't validate the structure or types of the values. If you need
  schema validation, use `Output.object()` or `Output.array()` instead.
</Note>

## Error Handling

When `generateText` with structured output cannot generate a valid object, it throws a [`NoObjectGeneratedError`](/docs/reference/ai-toolkit-errors/ai-no-object-generated-error).
...
- [Generating Structured Data](/docs/ai-toolkit-core/generating-structured-data)
- [`generateText()`](/docs/reference/ai-toolkit-core/generate-text)
- [`streamText()`](/docs/reference/ai-toolkit-core/stream-text)
- [`zod-schema`](/docs/reference/ai-toolkit-core/zod-schema)
- [`json-schema`](/docs/reference/ai-toolkit-core/json-schema)
