...
```tsx
import { Message } from '@ai-toolkit/react';
...
<Note>
  ...
  parts](https://sdk.khulnasoft.com/docs/foundations/prompts#multi-modal-messages).
...
> **Note:** The URL can also be a data URL, which is a base64-encoded string that represents the content of a file. Currently, only `image/*` content types get automatically converted into [multi-modal content parts](https://sdk.khulnasoft.com/docs/foundations/prompts#multi-modal-messages). You will need to handle other content types manually.
