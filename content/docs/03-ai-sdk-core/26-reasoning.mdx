...
Many language models support an internal "reasoning" phase (sometimes also called "thinking") before producing a response. The AI TOOLKIT provides a top-level `reasoning` parameter on [`generateText`](/docs/reference/ai-toolkit-core/generate-text) and [`streamText`](/docs/reference/ai-toolkit-core/stream-text) that controls this behavior across providers with a single, portable setting.
