...
Provider options let you pass provider-specific configuration that goes beyond the [standard settings](/docs/ai-toolkit-core/settings) shared by all providers. They are set via the `providerOptions` property on functions like `generateText` and `streamText`.
...
<Note>
  ...
  parameter](/docs/ai-toolkit-core/reasoning) instead of provider-specific options.
...
- [OpenAI provider options](/providers/ai-toolkit-providers/openai)
- [Anthropic provider options](/providers/ai-toolkit-providers/anthropic)
...
Provider options work the same way when using the [Vercel AI Gateway](/providers/ai-toolkit-providers/ai-gateway). Use the underlying provider name (e.g. `openai`, `anthropic`) as the key — not `gateway`. The AI Gateway forwards these options to the target provider automatically.
...
For more on gateway routing, fallbacks, and other gateway-specific options, see the [AI Gateway provider documentation](/providers/ai-toolkit-providers/ai-gateway#provider-options).
...
- [OpenAI Provider](/providers/ai-toolkit-providers/openai)
- [Anthropic Provider](/providers/ai-toolkit-providers/anthropic)
