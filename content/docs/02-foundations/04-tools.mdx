...
- **`parameters`**: A [Zod schema](/docs/foundations/tools#schema-specification-and-validation-with-zod) or a [JSON schema](/docs/reference/ai-toolkit-core/json-schema) that defines the parameters. The schema is consumed by the LLM, and also used to validate the LLM tool calls.
...
Schemas are used to define the parameters for tools and to validate the [tool calls](/docs/ai-toolkit-core/tools-and-tool-calling).

The AI TOOLKIT supports both raw JSON schemas (using the [`jsonSchema` function](/docs/reference/ai-toolkit-core/json-schema))
and [Zod](https://zod.dev/) schemas (either directly or using the [`zodSchema` function](/docs/reference/ai-toolkit-core/zod-schema)).
...
<Note>
  ...
  [`generateObject`](/docs/reference/ai-toolkit-core/generate-object) and
  [`streamObject`](/docs/reference/ai-toolkit-core/stream-object).
</Note>
...
- **[browserbase](https://docs.browserbase.com/integrations/khulnasoft-ai/introduction)** - Browser tool that runs a headless browser
...
- **[Toolhouse](https://docs.toolhouse.ai/toolhouse/using-khulnasoft-ai)** - AI function-calling in 3 lines of code for over 25 different actions.
- **[Agent Tools](https://ai-toolkit-agents.vercel.app/?item=introduction)** - A collection of tools for agents.
...
- **[Interlify](https://www.interlify.com/docs/integrate-with-khulnasoft-ai)** - Convert APIs into tools so that AI can connect to your backend in minutes.
...
<Note>
  ...
  request](https://github.com/khulnasoft/ai/pulls) to add them to this list.
</Note>
