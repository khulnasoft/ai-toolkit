---
title: Coding Agents
description: Learn how to set up the AI TOOLKIT for use with coding agents, including installing skills, accessing bundled docs, and using DevTools.
---

# Getting Started with Coding Agents

This page explains how to get the most out of the AI TOOLKIT when working inside a coding agent (such as Claude Code, Codex, OpenCode, Cursor, or any other AI-assisted development environment).

## Install the AI TOOLKIT Skill

The fastest way to give your coding agent deep knowledge of the AI TOOLKIT is to install the official AI TOOLKIT skill. Skills are lightweight markdown files that load specialized instructions into your agent's context on demand — so your agent knows exactly how to use the SDK without you needing to explain it.

Install the AI TOOLKIT skill using `npx skills add`:
...
Once installed, any agent that supports the [Agent Skills](https://agentskills.io) format will automatically discover and load the skill when working on AI TOOLKIT tasks.
...
Once you've installed the `ai` package, you already have the full AI TOOLKIT documentation and source code available locally inside `node_modules`. Your coding agent can read these directly — no internet access required.
...
AI TOOLKIT DevTools gives you full visibility into your AI TOOLKIT calls during development. It captures LLM requests, responses, tool calls, token usage, and multi-step interactions, and displays them in a local web UI.

<Note type="warning">
  AI TOOLKIT DevTools is experimental and intended for local development only. Do
...
<div className="my-4">
  <Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
    <Tab>
      <Snippet text="pnpm add @ai-toolkit/devtools" dark />
    </Tab>
    <Tab>
      <Snippet text="npm install @ai-toolkit/devtools" dark />
    </Tab>
    <Tab>
      <Snippet text="yarn add @ai-toolkit/devtools" dark />
    </Tab>
    <Tab>
      <Snippet text="bun add @ai-toolkit/devtools" dark />
    </Tab>
  </Tabs>
</div>
...
Wrap your language model with the DevTools middleware using [`wrapLanguageModel`](/docs/ai-toolkit-core/middleware):
...
Use the wrapped model with any AI TOOLKIT Core function:
...
Open [http://localhost:4983](http://localhost:4983) to inspect your AI TOOLKIT interactions in real time.
...
- Read the [DevTools reference](/docs/ai-toolkit-core/devtools) for a complete list of captured data and configuration options.
- Explore [Tools and Tool Calling](/docs/ai-toolkit-core/tools-and-tool-calling) to build agents that can take real-world actions.
