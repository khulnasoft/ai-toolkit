---
title: Node.js
description: Welcome to the AI TOOLKIT quickstart guide for Node.js!
---
...
Install `ai-toolkit` and `@ai-toolkit/openai`, the AI TOOLKIT's OpenAI provider, along with other necessary dependencies.
...
```bash
pnpm add ai @ai-toolkit/openai zod dotenv
...
- Call [`streamText`](/docs/reference/ai-toolkit-core/stream-text), which is imported from the `ai-toolkit` package. This function accepts a configuration object that contains a `model` provider and `messages`.
...
You should see a prompt in your terminal. Test it out by entering a message and see the AI chatbot respond in real-time! The AI TOOLKIT makes it fast and easy to build AI chat interfaces with Node.js.
...
1. You import the `tool` function from the `ai-toolkit` package.
