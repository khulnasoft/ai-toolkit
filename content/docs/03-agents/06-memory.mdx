...
You can add memory to your agent with the AI TOOLKIT in three ways, each with different tradeoffs:
...
Another approach is to use a provider that has memory built in. These providers wrap an external memory service and expose it through the AI TOOLKIT's standard interface. Memory storage, retrieval, and injection happen transparently, and you do not define any tools yourself.

### Letta

[Letta](https://letta.com) provides agents with persistent long-term memory. You create an agent on Letta's platform (cloud or self-hosted), configure its memory there, and use the AI TOOLKIT provider to interact with it. Letta's agent runtime handles memory management (core memory, archival memory, recall).

```bash
pnpm add @letta-ai/vercel-ai-toolkit-provider
```

```ts
import { lettaCloud } from '@letta-ai/vercel-ai-toolkit-provider';
import { ToolLoopAgent } from 'ai';
...
```ts
__PROVIDER_IMPORT__;
import { supermemoryTools } from '@supermemory/tools/ai-toolkit';
import { ToolLoopAgent } from 'ai';
...
Supermemory works with any AI TOOLKIT provider. The tools give the model `addMemory` and `searchMemories` operations that handle storage and retrieval.
...
```bash
pnpm add @vectorize-io/hindsight-ai-toolkit @vectorize-io/hindsight-client
```

```ts
__PROVIDER_IMPORT__;
import { HindsightClient } from '@vectorize-io/hindsight-client';
import { createHindsightTools } from '@vectorize-io/hindsight-ai-toolkit';
...
The `bankId` identifies the memory store and is typically a user ID. In multi-user apps, call `createHindsightTools` inside your request handler so each request gets the right bank. Hindsight works with any AI TOOLKIT provider.
