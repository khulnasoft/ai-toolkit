...
The AI TOOLKIT provides built-in loop control through two parameters: `stopWhen` for defining stopping conditions and `prepareStep` for modifying settings (model, tools, messages, and more) between steps.
...
The AI TOOLKIT provides several built-in stopping conditions:

```ts
import { ToolLoopAgent, stepCountIs } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
```ts
import { ToolLoopAgent, stepCountIs, hasToolCall } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
```ts
import { ToolLoopAgent, StopCondition, ToolSet } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
For scenarios requiring complete control over the agent loop, you can use AI TOOLKIT Core functions (`generateText` and `streamText`) to implement your own loop management instead of using `stopWhen` and `prepareStep`. This approach provides maximum flexibility for complex workflows.
