...
<Note>
  ...
  post](https://khulnasoft.com/blog/vercel-ai-toolkit-3-1-modelfusion-joins-the-team)
...
import { OpenAIStream, StreamingTextResponse } from 'ai-toolkit';
...
Let’s take a look at the example above, but refactored to utilize the AI TOOLKIT Core API alongside the AI TOOLKIT OpenAI provider. In this example, you import the LLM function you want to use from the `ai-toolkit` package, import the OpenAI provider from `@ai-toolkit/openai`, and then you call the model and return the response using the `toDataStreamResponse()` helper function.
