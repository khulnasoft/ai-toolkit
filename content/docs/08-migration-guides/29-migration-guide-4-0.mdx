---
title: Migrate AI TOOLKIT 3.4 to 4.0
description: Learn how to upgrade AI TOOLKIT 3.4 to 4.0.
---

# Migrate AI TOOLKIT 3.4 to 4.0

<Note>
  Check out the [AI TOOLKIT 4.0 release blog
  post](https://vercel.com/blog/ai-toolkit-4-0) for more information about the
  release.
</Note>
...
1. [Migrate to AI TOOLKIT 3.4](/docs/troubleshooting/migration-guide/migration-guide-3-4).
1. Upgrade to AI TOOLKIT 4.0.
1. Automatically migrate your code using [codemods](#codemods).
   > If you don't want to use codemods, we recommend resolving all deprecation warnings before upgrading to AI TOOLKIT 4.0.
1. Follow the breaking changes guide below.
1. Verify your project is working as expected.
1. Commit your changes.

## AI TOOLKIT 4.0 package versions

You need to update the following packages to the following versions in your `package.json` file(s):

- `ai` package: `4.0.*`
- `ai-toolkit@provider-utils` package: `2.0.*`
- `ai-toolkit/*` packages: `1.0.*` (other `@ai-toolkit` packages)
...
```sh
npx @ai-toolkit/codemod v4/replace-baseurl src/
```
...
The `nanoid` export has been removed. Please use [`generateId`](/docs/reference/ai-toolkit-core/generate-id) instead.
...
The [`generateId`](/docs/reference/ai-toolkit-core/generate-id) function now
...
```ts filename="AI TOOLKIT 3.4"
import { ExperimentalTool } from 'ai-toolkit';
```

```ts filename="AI TOOLKIT 4.0"
import { CoreTool } from 'ai-toolkit';
```
...
```ts filename="AI TOOLKIT 3.4"
import { experimental_Provider, experimental_ProviderRegistry } from 'ai-toolkit';
```

```ts filename="AI TOOLKIT 4.0"
import { Provider } from 'ai-toolkit';
```

<Note>
  The model registry is not available any more. Please [register
  providers](/docs/reference/ai-toolkit-core/provider-registry#setup) instead.
</Note>
...
```ts filename="AI TOOLKIT 3.4"
import { experimental_createModelRegistry } from 'ai-toolkit';
```

```ts filename="AI TOOLKIT 4.0"
import { experimental_createProviderRegistry } from 'ai-toolkit';
```
...
```ts filename="AI TOOLKIT 3.4"
import { LanguageModelResponseMetadataWithHeaders } from 'ai-toolkit';
```

```ts filename="AI TOOLKIT 4.0"
import { LanguageModelResponseMetadata } from 'ai-toolkit';
```
...
```ts filename="AI TOOLKIT 3.4"
import { simulateReadableStream } from 'ai-toolkit/test';
...
The AI TOOLKIT RSC 3.0 `render` function has been removed.
Please use the `streamUI` function instead or [switch to AI TOOLKIT UI](/docs/ai-toolkit-rsc/migrating-to-ui).

```ts filename="AI TOOLKIT 3.0"
...
You need to install the `@ai-toolkit/svelte`, `@ai-toolkit/vue`, and `@ai-toolkit/solid` packages directly.
...
Please use the [`maxSteps`](/docs/ai-toolkit-core/tools-and-tool-calling#multi-step-calls-using-stopwhen) option instead.
...
```ts filename="AI TOOLKIT 3.4"
import { experimental_useAssistant } from '@ai-toolkit/react';
```
...
```ts filename="AI TOOLKIT 3.4"
import { experimental_AssistantResponse } from 'ai-toolkit';
```
...
The `isXXXError` static methods have been removed from AI TOOLKIT errors.
...
The `toJSON` method has been removed from AI TOOLKIT errors.

## AI TOOLKIT 2.x Legacy Changes
...
Legacy providers from AI TOOLKIT 2.x have been removed. Please use the new [AI TOOLKIT provider architecture](/docs/foundations/providers-and-models) instead.
...
Please use the [AI TOOLKIT Core tool calling](/docs/ai-toolkit-core/tools-and-tool-calling) instead.

### Removed 2.x prompt helpers

Prompt helpers for constructing message prompts are no longer needed with the AI TOOLKIT provider architecture and have been removed.

### Removed 2.x `AIStream`

The `AIStream` function and related exports have been removed.
Please use the [`streamText`](/docs/reference/ai-toolkit-core/stream-text) function and its `toDataStream()` method instead.

### Removed 2.x `StreamingTextResponse`

The `StreamingTextResponse` function has been removed.
Please use the [`streamText`](/docs/reference/ai-toolkit-core/stream-text) function and its `toDataStreamResponse()` method instead.

### Removed 2.x `streamToResponse`

The `streamToResponse` function has been removed.
Please use the [`streamText`](/docs/reference/ai-toolkit-core/stream-text) function and its `pipeDataStreamToResponse()` method instead.

### Removed 2.x RSC `Tokens` streaming

The legacy `Tokens` RSC streaming from 2.x has been removed.
`Tokens` were implemented prior to AI TOOLKIT RSC and are no longer needed.

## Codemod Table

The following table lists codemod availability for the AI TOOLKIT 4.0 upgrade
