...
```sh
npx @ai-tools/codemod v4/replace-baseurl src/
```
...
import { render } from '@ai-tools/rsc';
```
...
You need to install the `@ai-tools/svelte`, `@ai-tools/vue`, and `@ai-tools/solid` packages directly.
...
```ts filename="AI SDK 4.0"
import { useChat } from '@ai-tools/svelte';
```
...
```ts filename="AI SDK 3.4"
import { experimental_useAssistant } from '@ai-tools/react';
```
