---
title: Migrate Your Data to AI TOOLKIT 5.0
description: Learn how to migrate your persisted messages and chat data from AI TOOLKIT 4.x to 5.0.
---

# Migrate Your Data to AI TOOLKIT 5.0

AI TOOLKIT 5.0 introduces changes to the message structure and persistence patterns. Unlike code migrations that can often be automated with codemods, data migration depends on your specific persistence approach, database schema, and application requirements.

**This guide helps you get your application working with AI TOOLKIT 5.0 first** using a runtime conversion layer. This allows you to update your app immediately without database migrations blocking you. You can then migrate your data schema at your own pace.
...
**Goal:** Update your application to AI TOOLKIT 5.0 without touching your database.
...
4. Update the rest of your application code to AI TOOLKIT 5.0 (see the [main migration guide](/docs/migration-guides/migration-guide-5-0))
...
Before starting, understand the main persistence-related changes in AI TOOLKIT 5.0:
...
import type { ToolUIPart, UIMessage, UITools } from 'ai-toolkit';
...
import { streamText, generateId, convertToModelMessages } from 'ai-toolkit';
__PROVIDER_IMPORT__;
...
**What's next:** Follow the main migration guide to update the rest of your application code to AI TOOLKIT 5.0, including API routes, components, and other code that uses the AI TOOLKIT. Then proceed to Phase 2.

See the [main migration guide](/docs/migration-guides/migration-guide-5-0) for details.

## Phase 2: Side-by-Side Schema Migration

Now that your application is updated to AI TOOLKIT 5.0 and working with the runtime conversion layer from Phase 1, you have a fully functional system. However, **the conversion functions are only a temporary solution**. Your database still stores messages in the v4 format, which means:
...
- [AI TOOLKIT Migration: Handling Previously Saved Messages](https://jhakim.com/blog/ai-toolkit-migration-handling-previously-saved-messages) - Detailed transformation function implementation
- [How we migrated Atypica.ai to AI TOOLKIT v5 without breaking 10M+ chat histories](https://blog.web3nomad.com/p/how-we-migrated-atypicaai-to-ai-toolkit-v5-without-breaking-10m-chat-histories) - Runtime conversion approach for large-scale migration
