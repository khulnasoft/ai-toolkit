...
- `ai` package: `^6.0.0`
- `@ai-tools/provider` package: `^3.0.0`
- `@ai-tools/provider-utils` package: `^4.0.0`
- `@ai-tools/*` packages: `^3.0.0`

An example upgrade command would be:

```
pnpm install ai@latest @ai-tools/react@latest @ai-tools/openai@latest
```
...
<Note>
  There is also an `npx @ai-tools/codemod upgrade` command, but it runs all
...
```sh
npx @ai-tools/codemod v6/rename-text-embedding-to-embedding src/
```
...
The `@ai-tools/azure` provider now uses the Responses API by default when calling `azure()` ([PR #9868](https://github.com/khulnasoft/ai-toolkit/pull/9868)). To use the previous Chat Completions API behavior, use `azure.chat()` instead.
...
For the **Responses API**, the `@ai-tools/azure` provider now uses `azure` as the key for `providerMetadata` and `providerOptions` instead of `openai`. The `openai` key is still supported for `providerOptions` input, but resulting `providerMetadata` output now uses `azure`.
...
Anthropic has [ introduced native structured outputs for Claude Sonnet 4.5 and later models ](https://www.claude.com/blog/structured-outputs-on-the-claude-developer-platform). The `@ai-tools/anthropic` provider now includes a `structuredOutputMode` option to control how structured outputs are generated ([PR #10502](https://github.com/khulnasoft/ai-toolkit/pull/10502)).
...
The `@ai-tools/google-vertex` provider now uses `vertex` as the key for `providerMetadata` and `providerOptions` instead of `google`. The `google` key is still supported for `providerOptions` input, but resulting `providerMetadata` output now uses `vertex`.
