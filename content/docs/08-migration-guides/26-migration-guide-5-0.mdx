...
- `ai` package: `5.0.0`
- `@ai-tools/provider` package: `2.0.0`
- `@ai-tools/provider-utils` package: `3.0.0`
- `@ai-tools/*` packages: `2.0.0` (other `@ai-sdk` packages)
...
```
npm install ai @ai-tools/react @ai-tools/openai zod@^4.1.8
```
...
```sh
npx @ai-tools/codemod v5/rename-format-stream-part src/
```

See also the [table of codemods](#codemod-table). In addition, the latest set of
codemods can be found in the
[`@ai-tools/codemod`](https://github.com/khulnasoft/ai-toolkit/tree/main/packages/codemod/src/codemods)
repository.
...
### `@ai-tools/rsc` Package Extraction
...
<Note>Don't forget to install the new package: `npm install @ai-tools/rsc`</Note>

### React UI Hooks Moved to `@ai-tools/react`

The deprecated `ai/react` export has been removed in favor of `@ai-tools/react`.
...
<Note>
  Don't forget to install the new package: `npm install @ai-tools/react`
</Note>
...
```tsx filename="AI SDK 4.0"
import { useChat, type Message } from '@ai-tools/react';
...
```tsx filename="AI SDK 5.0"
import { useChat, type UIMessage } from '@ai-tools/react';
...
```typescript filename="@ai-tools/vue v1"
<script setup>
import { useChat } from '@ai-tools/vue';
...
```typescript filename="@ai-tools/vue v2"
<script setup>
...
```js filename="@ai-tools/svelte v1"
...
```js filename="@ai-tools/svelte v2"
import { Chat } from '@ai-tools/svelte';
import { DefaultChatTransport } from 'ai';
...
#### `@ai-tools/ui-utils` Package Removal

The `@ai-tools/ui-utils` package has been removed and its exports moved to the main `ai` package.

```tsx filename="AI SDK 4.0"
import { getTextFromDataUrl } from '@ai-tools/ui-utils';
```
...
```tsx filename="AI SDK 4.0"
import { useAssistant } from '@ai-tools/react';
```
...
#### LangChain Adapter Moved to `@ai-tools/langchain`

The `LangChainAdapter` has been moved to `@ai-tools/langchain` and the API has been updated to use UI message streams.
...
<Note>
  Don't forget to install the new package: `npm install @ai-tools/langchain`
</Note>

#### LlamaIndex Adapter Moved to `@ai-tools/llamaindex`

The `LlamaIndexAdapter` has been extracted to a separate package `@ai-tools/llamaindex` and follows the same UI message stream pattern.
...
```tsx filename="AI SDK 5.0"
import { toUIMessageStream } from '@ai-tools/llamaindex';
import { createUIMessageStreamResponse } from 'ai';
...
<Note>
  Don't forget to install the new package: `npm install @ai-tools/llamaindex`
</Note>
...
`LanguageModelV3` must now be imported from `@ai-tools/provider`.
...
```tsx filename="AI SDK 5.0"
import { LanguageModelV3 } from '@ai-tools/provider';
```
...
```tsx filename="AI SDK 5.0"
import { LanguageModelV3Middleware } from '@ai-tools/provider';
```
...
import { openai, type OpenAIResponsesProviderOptions } from '@ai-tools/openai';
