---
title: Tool Calling
description: Learn about tool calling and multi-step calls (using maxSteps) with AI TOOLKIT Core.
---
...
AI TOOLKIT Core tools contain three elements:

- **`description`**: An optional description of the tool that can influence when the tool is picked.
- **`parameters`**: A [Zod schema](/docs/foundations/tools#schemas) or a [JSON schema](/docs/reference/ai-toolkit-core/json-schema) that defines the parameters. The schema is consumed by the LLM, and also used to validate the LLM tool calls.
...
With the `maxSteps` setting, you can enable multi-step calls in `generateText` and `streamText`. When `maxSteps` is set to a number greater than 1 and the model generates a tool call, the AI TOOLKIT will trigger a new generation passing in the tool result until there
...
- [`InvalidToolArgumentsError`](/docs/reference/ai-toolkit-errors/ai-invalid-tool-arguments-error): the model calls a tool with arguments that do not match the tool's parameters
- [`ToolExecutionError`](/docs/reference/ai-toolkit-errors/ai-tool-execution-error): an error that occurred during tool execution
...
the AI TOOLKIT provides the `experimental_activeTools` property.
...
AI TOOLKIT Core tools have an optional `experimental_toToolResultContent` function
...
The AI TOOLKIT supports connecting to [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) servers to access their tools.
...
The client's `tools` method acts as an adapter between MCP tools and AI TOOLKIT tools. It supports two approaches for working with tool schemas:
