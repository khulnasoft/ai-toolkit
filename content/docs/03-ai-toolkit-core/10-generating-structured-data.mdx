...
description: Learn how to generate structured data with the AI TOOLKIT.
---
...
The AI TOOLKIT standardises structured object generation across model providers
using the `output` property on [`generateText`](/docs/reference/ai-toolkit-core/generate-text)
and [`streamText`](/docs/reference/ai-toolkit-core/stream-text).
You can use [Zod schemas](/docs/reference/ai-toolkit-core/zod-schema), [Valibot](/docs/reference/ai-toolkit-core/valibot-schema), or [JSON schemas](/docs/reference/ai-toolkit-core/json-schema) to specify the shape of the data that you want,
...
<Note>
  Structured output generation counts as a step in the AI TOOLKIT's multi-turn
...
You can consume the structured output on the client with the [`useObject`](/docs/reference/ai-toolkit-ui/use-object) hook.
...
The AI TOOLKIT supports multiple ways of specifying the expected structure of generated data via the `Output` object. You can select from various strategies for structured/text generation and validation.
...
You can provide any set of string options, and the output will always be a single string value that matches one of the specified options. The AI TOOLKIT validates that the result matches one of your options, and will throw if the model returns something invalid.
...
With `Output.json`, the AI TOOLKIT only checks that the response is valid JSON; it doesn't validate the structure or types of the values. If you need schema validation, use the `.object` or `.array` outputs instead.

For more advanced validation or different structures, see [the Output API reference](/docs/reference/ai-toolkit-core/output).
...
```ts
import { generateText, Output, tool, stepCountIs } from 'ai-toolkit';
__PROVIDER_IMPORT__;
import { z } from 'zod';
...
When `generateText` with structured output cannot generate a valid object, it throws a [`AI_NoObjectGeneratedError`](/docs/reference/ai-toolkit-errors/ai-no-object-generated-error).
...
```ts
import { generateText, Output, NoObjectGeneratedError } from 'ai-toolkit';
...
<ExampleLinks
  examples={[
    ...
    {
      title:
        'Learn to generate objects in Next.js with Route Handlers (AI TOOLKIT UI)',
      link: '/examples/next-pages/basics/generating-object',
    },
    {
      title:
        'Learn to generate objects in Next.js with Server Actions (AI TOOLKIT RSC)',
...
<ExampleLinks
  examples={[
    ...
    {
      title:
        'Learn to stream objects in Next.js with Route Handlers (AI TOOLKIT UI)',
      link: '/examples/next-pages/basics/streaming-object-generation',
    },
    {
      title:
        'Learn to stream objects in Next.js with Server Actions (AI TOOLKIT RSC)',
