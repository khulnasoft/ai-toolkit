...
- `MockEmbeddingModelV3`: A mock embedding model using the [embedding model v3 specification](https://github.com/khulnasoft/ai-toolkit/blob/main/packages/validation/provider/src/embedding-model/v3/embedding-model-v3.ts).
- `MockLanguageModelV3`: A mock language model using the [language model v3 specification](https://github.com/khulnasoft/ai-toolkit/blob/main/packages/validation/provider/src/language-model/v3/language-model-v3.ts).
...
```ts
import { streamObject, simulateReadableStream } from 'ai-toolkit';
import { MockLanguageModelV3 } from 'ai-toolkit/test';
import { z } from 'zod';
...
You can also simulate [UI Message Stream](/docs/ai-toolkit-ui/stream-protocol#ui-message-stream-example) responses for testing,
