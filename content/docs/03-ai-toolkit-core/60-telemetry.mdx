---
title: Telemetry
description: Using OpenTelemetry with AI TOOLKIT Core
---

# Telemetry

<Note type="warning">
  AI TOOLKIT Telemetry is experimental and may change in the future.
</Note>

The AI TOOLKIT uses [OpenTelemetry](https://opentelemetry.io/) to collect telemetry data.
...
Check out the [AI TOOLKIT Observability Integrations](/providers/observability)
to see providers that offer monitoring and tracing for AI TOOLKIT applications.
...
import { devToolsIntegration } from '@ai-toolkit/devtools';
...
The event types for each method are the same as the corresponding [event callbacks](/docs/ai-toolkit-core/event-listeners). See the event callbacks documentation for the full property reference of each event.
