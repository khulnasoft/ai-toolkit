...
description: Learn how to call tools in parallel using the AI TOOLKIT and Next.js
...
You will create a new route at `/api/chat` that will use the `streamText` function from the `ai-toolkit` module to generate the assistant's response based on the conversation history.

You will use the [`tools`](/docs/reference/ai-toolkit-core/generate-text#tools) parameter to specify a tool called `getWeather` that will get the weather for a location.
...
<GithubLink link="https://github.com/khulnasoft/ai-toolkit/blob/main/examples/next-openai-pages/pages/tools/call-tools-in-parallel/index.tsx" />
