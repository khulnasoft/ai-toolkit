---
title: Stream Text
description: Learn how to stream text using the AI TOOLKIT and Next.js
tags: ['next', 'streaming']
---
...
Let's create a route handler for `/api/completion` that will generate text based on the input prompt. The route will call the `streamText` function from the `ai-toolkit` module, which will then generate text based on the input prompt and stream it to the client.
...
<GithubLink link="https://github.com/khulnasoft/ai-toolkit/blob/main/examples/next-openai-pages/pages/basics/stream-text/index.tsx" />
