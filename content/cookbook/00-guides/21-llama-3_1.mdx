...
At the center of the AI SDK is [AI SDK Core](/docs/ai-toolkit-core/overview), which provides a unified API to call any LLM. The code snippet below is all you need to call Llama 3.1 (using [DeepInfra](https://deepinfra.com)) with the AI SDK:
...
AI SDK Core abstracts away the differences between model providers, allowing you to focus on building great applications. Prefer to use [Amazon Bedrock](/providers/ai-toolkit-providers/amazon-bedrock)? The unified interface also means that you can easily switch between models by changing just two lines of code.
...
While text generation can be useful, you might want to generate structured JSON data. For example, you might want to extract information from text, classify data, or generate synthetic data. AI SDK Core provides [`generateText`](/docs/reference/ai-toolkit-core/generate-text) and [`streamText`](/docs/reference/ai-toolkit-core/stream-text) with `Output` to generate structured data, allowing you to constrain model outputs to a specific schema.
...
The AI SDK supports tool usage across several of its functions, including [`generateText`](/docs/reference/ai-toolkit-core/generate-text) and [`streamUI`](/docs/reference/ai-toolkit-rsc/stream-ui). By passing one or more tools to the `tools` parameter, you can extend the capabilities of LLMs, allowing them to perform discrete tasks and interact with external systems.
...
AI SDK Core can be paired with [AI SDK UI](/docs/ai-toolkit-ui/overview), another powerful component of the AI SDK, to streamline the process of building chat, completion, and assistant interfaces with popular frameworks like Next.js, Nuxt, and SvelteKit.
...
The AI SDK's React Server Components (RSC) API enables you to create rich, interactive interfaces that go beyond simple text generation. With the [`streamUI`](/docs/reference/ai-toolkit-rsc/stream-ui) function, you can dynamically stream React components from the server to the client.

Let's dive into how you can leverage tools with [AI SDK RSC](/docs/ai-toolkit-rsc/overview) to build a generative user interface with Next.js (App Router).
...
1. Explore the documentation at [ai-toolkit.dev/docs](/docs) to understand the full capabilities of the AI SDK.
