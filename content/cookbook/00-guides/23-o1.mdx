...
The [AI TOOLKIT](/) is a powerful TypeScript toolkit for building AI applications with large language models (LLMs) like OpenAI o1 alongside popular frameworks like React, Next.js, Vue, Svelte, Node.js, and more.
...
At the center of the AI TOOLKIT is [AI TOOLKIT Core](/docs/ai-toolkit-core/overview), which provides a unified API to call any LLM. The code snippet below is all you need to call OpenAI o1 with the AI TOOLKIT:
...
<Note>
  To use the o1 model, you must either be using @ai-toolkit/openai version
  0.0.59 or greater, or set `temperature: 1`.
</Note>

AI TOOLKIT Core abstracts away the differences between model providers, allowing you to focus on building great applications. The unified interface also means that you can easily switch between models by changing just one line of code.
...
The AI TOOLKIT supports tool usage across several of its functions, like [`generateText`](/docs/reference/ai-toolkit-core/generate-text) and [`streamText`](/docs/reference/ai-toolkit-core/stream-text). By passing one or more tools to the `tools` parameter, you can extend the capabilities of LLMs, allowing them to perform discrete tasks and interact with external systems.

Here's an example of how you can use a tool with the AI TOOLKIT and o1:
...
Let's explore building a chatbot with [Next.js](https://nextjs.org), the AI TOOLKIT, and OpenAI o1:
...
1. Check out our support for the o1 series of reasoning models in the [OpenAI Provider](/providers/ai-toolkit-providers/openai#reasoning-models).
1. Check out practical examples at [studio.khulnasoft.com/examples](/examples) to see the SDK in action and get inspired for your own projects.
1. Dive deeper with advanced guides on topics like Retrieval-Augmented Generation (RAG) and multi-modal chat at [studio.khulnasoft.com/docs/guides](/docs/guides).
