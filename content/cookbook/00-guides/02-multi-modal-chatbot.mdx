...
<Note>
  ...
  [providers documentation](/providers/ai-toolkit-providers).
</Note>
...
Install `ai` and `@ai-toolkit/react`, the AI SDK package and the AI SDK's React package respectively.
...
3. Call [`streamText`](/docs/reference/ai-toolkit-core/stream-text), which is imported from the `ai` package. This function accepts a configuration object that contains a `model` provider and `messages` (converted in step 2). You can pass additional [settings](/docs/ai-toolkit-core/settings) to further customize the model's behavior.
...
Now that you have a Route Handler that can query a large language model (LLM), it's time to setup your frontend. [ AI SDK UI ](/docs/ai-toolkit-ui) abstracts the complexity of a chat interface into one hook, [`useChat`](/docs/reference/ai-toolkit-ui/use-chat).
...
You've built a multi-modal AI agent using the AI SDK! Experiment and extend the functionality of this application further by exploring [tool calling](/docs/ai-toolkit-core/tools-and-tool-calling).
