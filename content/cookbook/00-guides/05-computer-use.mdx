...
description: Get started with Claude's Computer Use capabilities with the AI TOOLKIT
...
The AI TOOLKIT is a powerful TypeScript toolkit for building AI applications with large language models (LLMs) like Anthropic's Claude alongside popular frameworks like React, Next.js, Vue, Svelte, Node.js, and more. In this guide, you will learn how to integrate Computer Use into your AI TOOLKIT applications.
...
3. **Execute the action and return results**
   ...
   The AI TOOLKIT processes Claude's request by running the selected tool. The results can then be sent back to Claude through a `tool_result` message.
...
Computer Use tools in the AI TOOLKIT are predefined interfaces that require your own implementation of the execution layer. While the SDK provides the type definitions and structure for these tools, you need to:
...
<Note>
  If you have never used the AI TOOLKIT before, start by following the [Getting
  Started guide](/docs/getting-started).
</Note>

<Note>
  For a working example of Computer Use implementation with Next.js and the AI
  SDK, check out our [AI TOOLKIT Computer Use
  Template](https://github.com/vercel-labs/ai-toolkit-computer-use).
</Note>

First, ensure you have the AI TOOLKIT and [Anthropic AI TOOLKIT provider](/providers/ai-toolkit-providers/anthropic) installed:

<Snippet text="pnpm add ai @ai-toolkit/anthropic" />

You can add Computer Use to your AI TOOLKIT applications using provider-defined-client tools. These tools accept various input parameters (like display height and width in the case of the computer tool) and then require that you define an execute function.

Here's how you could set up the Computer Tool with the AI TOOLKIT:
...
Finally, to send tool results back to the model, use the [`toModelOutput()`](/docs/foundations/prompts#multi-modal-tool-results) function to convert text and image responses into a format the model can process. The AI TOOLKIT includes experimental support for these multi-modal tool results when using Anthropic's models.
...
Once your tool is defined, you can use it with both the [`generateText`](/docs/reference/ai-toolkit-core/generate-text) and [`streamText`](/docs/reference/ai-toolkit-core/stream-text) functions.
...
You can combine multiple tools in a single request to enable more complex workflows. The AI TOOLKIT supports all three of Claude's Computer Use tools:
