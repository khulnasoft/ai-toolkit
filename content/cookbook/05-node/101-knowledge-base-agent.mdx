...
description: Build an AI agent that can read from and write to a knowledge base using Upstash Search and the AI TOOLKIT
tags: ['node', 'retrieval', 'tools']
---

In this recipe, you'll learn how to build an AI agent that can interact with a knowledge base using [Upstash Search](https://upstash.com/docs/search). The agent will be able to both retrieve information from the knowledge base and add new resources to it, leveraging AI TOOLKIT tools.
...
Install the AI TOOLKIT, OpenAI provider, Upstash Search packages, and tsx as a dev dependency:

```bash
pnpm i ai zod @ai-toolkit/openai @upstash/search
pnpm i -D tsx
```
...
import { tool, stepCountIs, generateText, generateId } from 'ai-toolkit';
import { z } from 'zod';
import { Search } from '@upstash/search';
