...
<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  ...
  <Tab>
    <Snippet text="bun add @ai-toolkit/mistral" dark />
  </Tab>
</Tabs>
...
These models return structured reasoning content that the AI TOOLKIT extracts automatically. The reasoning is available via the `reasoningText` property in the result:
...
You can use the AI TOOLKIT's top-level `reasoning` setting to control reasoning effort:
...
Mistral chat models support structured outputs using JSON Schema. You can use `generateText` or `streamText` with [`Output`](/docs/reference/ai-toolkit-core/output)
