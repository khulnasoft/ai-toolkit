...
The Google Vertex and Google Vertex Anthropic providers are both available in the `@ai-toolkit/google-vertex` module. You can install it with

<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  <Tab>
    <Snippet text="pnpm add @ai-toolkit/google-vertex" dark />
  </Tab>
  <Tab>
    <Snippet text="npm install @ai-toolkit/google-vertex" dark />
  </Tab>
  <Tab>
    <Snippet
      text="yarn add @ai-toolkit/google-vertex @google-cloud/vertexai"
      dark
    />
  </Tab>

  <Tab>
    <Snippet text="bun add @ai-toolkit/google-vertex" dark />
  </Tab>
</Tabs>
...
You can import the default provider instance `vertex` from `@ai-toolkit/google-vertex`:

```ts
import { vertex } from '@ai-toolkit/google-vertex';
```

If you need a customized setup, you can import `createVertex` from `@ai-toolkit/google-vertex` and create a provider instance with your settings:
...
The Node.js runtime is the default runtime supported by the AI TOOLKIT. It supports all standard Google Cloud authentication options through the [`google-auth-library`](https://github.com/googleapis/google-auth-library-nodejs?tab=readme-ov-file#ways-to-authenticate). Typical use involves setting a path to a json credentials file in the `GOOGLE_APPLICATION_CREDENTIALS` environment variable. The credentials file can be obtained from the [Google Cloud Console](https://console.cloud.google.com/apis/credentials).
...
You can import the default provider instance `vertex` from `@ai-toolkit/google-vertex/edge`:
...
<Note>
  The `/edge` sub-module is included in the `@ai-toolkit/google-vertex` package, so
  you don't need to install it separately. You must import from
  `@ai-toolkit/google-vertex/edge` to differentiate it from the Node.js provider.
</Note>

If you need a customized setup, you can import `createVertex` from `@ai-toolkit/google-vertex/edge` and create a provider instance with your settings:
...
of the [standard call settings](/docs/ai-toolkit-core/settings). You can pass them as
an options argument:
...
Google Vertex AI, through its support for Gemini models, can also emit "thinking" tokens, representing the model's reasoning process. The AI TOOLKIT exposes these as reasoning information.
...
<Note>
  The AI TOOLKIT will automatically download URLs if you pass them as data, except
...
First, create a cache using the Google GenAI TOOLKIT with Vertex mode enabled:
...
Then use the cache with the AI TOOLKIT:
...
```ts
import { vertex } from '@ai-toolkit/google-vertex';
import { GoogleVertexImageModelOptions } from '@ai-toolkit/google-vertex';
import { generateImage } from 'ai';
...
```ts
import { vertex, GoogleVertexImageModelOptions } from '@ai-toolkit/google-vertex';
import { generateImage } from 'ai';
import fs from 'fs';
...
using the `.video()` factory method. For more on video generation with the AI TOOLKIT see [generateVideo()](/docs/reference/ai-toolkit-core/generate-video).
...
```ts
import { vertex } from '@ai-toolkit/google-vertex';
import { GoogleVertexVideoModelOptions } from '@ai-toolkit/google-vertex';
import { experimental_generateVideo as generateVideo } from 'ai';
...
The Google Vertex Anthropic provider for the [AI TOOLKIT](/docs) offers support for Anthropic's Claude models through the Google Vertex AI APIs. This section provides details on how to set up and use the Google Vertex Anthropic provider.

### Provider Instance

You can import the default provider instance `vertexAnthropic` from `@ai-toolkit/google-vertex/anthropic`:

```typescript
import { vertexAnthropic } from '@ai-toolkit/google-vertex/anthropic';
```

If you need a customized setup, you can import `createVertexAnthropic` from `@ai-toolkit/google-vertex/anthropic` and create a provider instance with your settings:
...
For Edge runtimes, you can import the provider instance from `@ai-toolkit/google-vertex/anthropic/edge`:
...
```typescript
import { createVertexAnthropic } from '@ai-toolkit/google-vertex/anthropic/edge';
...
and support structured data generation with [`Output`](/docs/reference/ai-toolkit-core/output)
(see [AI TOOLKIT Core](/docs/ai-toolkit-core)).
...
See [AI TOOLKIT UI: Chatbot](/docs/ai-toolkit-ui/chatbot#reasoning) for more details
...
<Note>
  ...
  `@ai-toolkit/anthropic` provider if you need access to all Anthropic tools.
</Note>
