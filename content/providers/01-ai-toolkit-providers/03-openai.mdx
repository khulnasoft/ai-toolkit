...
<Note>
  ...
  documented in [MCP Tools](/docs/ai-toolkit-core/mcp-tools). The OpenAI MCP
  tool is a built-in provider-defined tool that allows OpenAI models to directly
  connect to MCP servers, while the general MCP client requires you to convert
  MCP tools to AI TOOLKIT tools first.
</Note>
...
```ts highlight="7"
import { openai, OpenAIChatLanguageModelOptions } from '@ai-toolkit/openai';
...
<Note type="warning">
  Because the `UIMessage` type (used by AI TOOLKIT UI hooks like `useChat`) does
  not support the `providerOptions` property, you can use
  `convertToModelMessages` first before passing the messages to functions like
  `generateText` or `streamText`. For more details on `providerOptions` usage,
  see [here](/docs/foundations/prompts#provider-options).
</Note>
