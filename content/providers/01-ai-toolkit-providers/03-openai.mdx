---
title: OpenAI
description: Learn how to use the OpenAI provider for the AI TOOLKIT.
---
...
If you need a customized setup, you can import `createOpenAI` from `@ai-toolkit/openai` and create a provider instance with your settings:
...
<Note>
  Since AI TOOLKIT 5, the OpenAI responses API is called by default (unless you
...
You can use the OpenAI responses API with the `openai(modelId)` or `openai.responses(modelId)` factory methods. It is the default API that is used by the OpenAI provider (since AI TOOLKIT 5).
...
```ts
import { openai, OpenAILanguageModelResponsesOptions } from '@ai-toolkit/openai';
import { generateText } from 'ai';
...
```ts
import { openai, type OpenaiResponsesProviderMetadata } from '@ai-toolkit/openai';
import { generateText } from 'ai';
...
The [`ai-toolkit-openai-websocket-fetch`](https://www.npmjs.com/package/ai-toolkit-openai-websocket-fetch)
package provides a drop-in `fetch` replacement that routes streaming requests
through a persistent WebSocket connection.

<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  <Tab>
    <Snippet text="pnpm add ai-toolkit-openai-websocket-fetch" dark />
  </Tab>
  <Tab>
    <Snippet text="npm install ai-toolkit-openai-websocket-fetch" dark />
  </Tab>
  <Tab>
    <Snippet text="yarn add ai-toolkit-openai-websocket-fetch" dark />
  </Tab>
  <Tab>
    <Snippet text="bun add ai-toolkit-openai-websocket-fetch" dark />
  </Tab>
</Tabs>
...
import { createOpenAI } from '@ai-toolkit/openai';
import { createWebSocketFetch } from 'ai-toolkit-openai-websocket-fetch';
import { streamText } from 'ai';
...
in the [demo app](https://github.com/vercel-labs/ai-toolkit-openai-websocket).
...
<Note>
  ...
  documented in [MCP Tools](/docs/ai-toolkit-core/mcp-tools). The OpenAI MCP tool is
...
The OpenAI Responses API supports structured outputs. You can use `generateText` or `streamText` with [`Output`](/docs/reference/ai-toolkit-core/output) to enforce structured outputs.
...
To enable type-safe handling of this metadata, the AI TOOLKIT exports dedicated TypeScript types.
...
OpenAI chat models support also some model specific provider options that are not part of the [standard call settings](/docs/ai-toolkit-core/settings).
You can pass them in the `providerOptions` argument:

```ts
import { openai, type OpenAILanguageModelChatOptions } from '@ai-toolkit/openai';
...
```ts highlight="7"
import { openai, OpenAILanguageModelChatOptions } from '@ai-toolkit/openai';
...
<Note type="warning">
  Because the `UIMessage` type (used by AI TOOLKIT UI hooks like `useChat`) does not
...
```ts
import { openai, type OpenAIEmbeddingModelOptions } from '@ai-toolkit/openai';
...
For more on `generateImage()` see [Image Generation](/docs/ai-toolkit-core/image-generation).
...
import { openai, type OpenAITranscriptionModelOptions } from '@ai-toolkit/openai';
...
```ts highlight="7-9"
import { experimental_generateSpeech as generateSpeech } from 'ai';
import { openai, type OpenAISpeechModelOptions } from '@ai-toolkit/openai';
