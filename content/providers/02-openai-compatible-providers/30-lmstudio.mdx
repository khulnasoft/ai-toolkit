...
The LM Studio provider is available via the `@ai-tools/openai-compatible` module as it is compatible with the OpenAI API.
...
To use LM Studio, you can create a custom provider instance with the `createOpenAICompatible` function from `@ai-tools/openai-compatible`:
