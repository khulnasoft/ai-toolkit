...
The Clarifai provider is available via the `@ai-tools/openai-compatible` module as it is compatible with the OpenAI API. You can install it with:
...
To use Clarifai, you can create a custom provider instance with the `createOpenAICompatible` function from `@ai-tools/openai-compatible`:
