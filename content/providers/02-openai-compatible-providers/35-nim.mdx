...
The NVIDIA NIM provider is available via the `@ai-tools/openai-compatible` module as it is compatible with the OpenAI API.
...
To use NVIDIA NIM, you can create a custom provider instance with the `createOpenAICompatible` function from `@ai-tools/openai-compatible`:
