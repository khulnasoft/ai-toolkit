---
title: Heroku
description: Use a Heroku OpenAI compatible API with the AI TOOLKIT.
---
...
You can deploy models that are OpenAI API compatible and use them with the AI TOOLKIT.

## Setup

The Heroku provider is available via the `@ai-toolkit/openai-compatible` module as it is compatible with the OpenAI API.
...
To use Heroku, you can create a custom provider instance with the `createOpenAICompatible` function from `@ai-toolkit/openai-compatible`:
...
Heroku language models also support structured data generation with [`Output`](/docs/reference/ai-toolkit-core/output).
