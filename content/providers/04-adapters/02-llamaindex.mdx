...
The AI SDK `@ai-toolkit/llamaindex` package uses the stream result from calling the `chat` method on a [LlamaIndex ChatEngine](https://ts.llamaindex.ai/modules/chat_engine) or the `query` method on a [LlamaIndex QueryEngine](https://ts.llamaindex.ai/modules/query_engines) to pipe text to the client.
...
Then, we use the AI SDK's [`useCompletion`](/docs/ai-toolkit-ui/completion) method in the page component to handle the completion:
