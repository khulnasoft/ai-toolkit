...
<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  ...
  <Tab>
    <Snippet text="bun add @ai-toolkit/google" dark />
  </Tab>
</Tabs>
...
<Note>
  Usage metadata was added to `providerMetadata` in `@ai-toolkit/google@1.2.23`. If
...
<Note>
  ...
  Vertex provider (`@ai-toolkit/google-vertex`) instead of the standard Google
  provider (`@ai-toolkit/google`) to use this feature. Requires Gemini 2.0 or newer
  models.
</Note>
...
```json
{
  "urlMetadata": [
    {
      "retrievedUrl": "https://ai-toolkit.dev/providers/ai-toolkit-providers/google-generative-ai",
...
"groundingMetadata": {
    "groundingChunks": [
        {
            "web": {
                "uri": "https://ai-toolkit.dev/providers/ai-toolkit-providers/google-generative-ai",
    ...
    "groundingSupports": [
        {
            "segment": {
                "startIndex": 67,
                "endIndex": 157,
                "text": "**Installation**: Install the `@ai-toolkit/google` module using your preferred package manager"
            },
...
const { text, sources, providerMetadata } = await generateText({
  model: google('gemini-2.5-flash'),
  prompt: `Based on this context: https://ai-toolkit.dev/providers/ai-toolkit-providers/google-generative-ai, tell me how to use Gemini with AI SDK.
...
<Note>
  ...
  the Google Vertex provider (`@ai-toolkit/google-vertex`) instead of the standard
  Google provider (`@ai-toolkit/google`) to use this feature.
</Note>
...
For more on image generation with the AI SDK see [generateImage()](/docs/reference/ai-toolkit-core/generate-image).
...
```ts
import { google } from '@ai-toolkit/google';
import { GoogleImageModelOptions } from '@ai-toolkit/google';
import { generateImage } from 'ai';
