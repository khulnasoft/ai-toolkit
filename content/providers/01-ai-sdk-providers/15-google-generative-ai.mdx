...
The Google provider is available in the `@ai-tools/google` module. You can install it with

<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  <Tab>
    <Snippet text="pnpm add @ai-tools/google" dark />
  </Tab>
  <Tab>
    <Snippet text="npm install @ai-tools/google" dark />
  </Tab>
  <Tab>
    <Snippet text="yarn add @ai-tools/google" dark />
  </Tab>

  <Tab>
    <Snippet text="bun add @ai-tools/google" dark />
  </Tab>
</Tabs>
...
If you need a customized setup, you can import `createGoogleGenerativeAI` from `@ai-tools/google` and create a provider instance with your settings:
...
<Note>
  Usage metadata was added to `providerMetadata` in `@ai-tools/google@1.2.23`. If
...
import { google } from '@ai-tools/google';
import { GoogleGenerativeAIProviderMetadata } from '@ai-tools/google';
import { generateText } from 'ai';
...
<Note>
  ...
  Vertex provider (`@ai-tools/google-vertex`) instead of the standard Google
  provider (`@ai-tools/google`) to use this feature. Requires Gemini 2.0 or newer
  models.
</Note>
    ...
    "groundingSupports": [
        {
            "segment": {
                "startIndex": 67,
                "endIndex": 157,
                "text": "**Installation**: Install the `@ai-tools/google` module using your preferred package manager"
            },
...
<Note>
  ...
  the Google Vertex provider (`@ai-tools/google-vertex`) instead of the standard
  Google provider (`@ai-tools/google`) to use this feature.
</Note>
...
```ts
import { google } from '@ai-tools/google';
import { GoogleImageModelOptions } from '@ai-tools/google';
import { generateImage } from 'ai';
