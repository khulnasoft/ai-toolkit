...
If you need a customized setup, you can import `createOpenAI` from `@ai-tools/openai` and create a provider instance with your settings:
...
```ts
import { openai, type OpenaiResponsesProviderMetadata } from '@ai-tools/openai';
import { generateText } from 'ai';
...
```ts highlight="7"
import { openai, OpenAILanguageModelChatOptions } from '@ai-tools/openai';
...
```ts
import { openai, type OpenAIEmbeddingModelOptions } from '@ai-tools/openai';
...
```ts highlight="7-9"
import { experimental_generateSpeech as generateSpeech } from 'ai';
import { openai, type OpenAISpeechModelOptions } from '@ai-tools/openai';
