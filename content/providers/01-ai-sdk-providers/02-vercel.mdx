...
The Vercel provider is available via the `@ai-tools/vercel` module. You can install it with:

<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  <Tab>
    <Snippet text="pnpm add @ai-tools/vercel" dark />
  </Tab>
  <Tab>
    <Snippet text="npm install @ai-tools/vercel" dark />
  </Tab>
  <Tab>
    <Snippet text="yarn add @ai-tools/vercel" dark />
  </Tab>

  <Tab>
    <Snippet text="bun add @ai-tools/vercel" dark />
  </Tab>
</Tabs>
...
If you need a customized setup, you can import `createVercel` from `@ai-tools/vercel` and create a provider instance with your settings:

```ts
import { createVercel } from '@ai-tools/vercel';
