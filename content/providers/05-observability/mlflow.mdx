---
title: MLflow
description: Track, visualize, and debug Vercel AI TOOLKIT traces with MLflow Tracing
---

# MLflow Observability

![MLflow Tracing Vercel AI TOOLKIT](https://mlflow.org/docs/latest/images/llms/tracing/vercel-ai-tracing.mp4)

[MLflow Tracing](https://mlflow.org/docs/latest/genai/tracing) provides automatic tracing for applications built with the [Vercel AI TOOLKIT](https://ai-toolkit.dev/) (the `ai` package) via OpenTelemetry, unlocking observability for TypeScript and JavaScript apps.
...
It is fairly straightforward to enable MLflow tracing for Vercel AI TOOLKIT if you are using NextJS.
...
Then enable telemetry where you call the AI TOOLKIT (for example in `route.ts`):
...
Start your NextJS app and open MLflow UI at the tracking server endpoint (e.g., `http://localhost:5000`). Traces for AI TOOLKIT calls appear in the configured experiment.
...
// Make an AI TOOLKIT call with telemetry enabled
...
To disable tracing for Vercel AI TOOLKIT, set `experimental_telemetry: { isEnabled: false }` on the AI TOOLKIT call.

## Learn more

- After setting up MLflow Tracing for the AI TOOLKIT, you can tap into broader MLflow GenAI capabilities:
...
- For more information about tracing in AI TOOLKIT, see the [telemetry documentation](/docs/ai-toolkit-core/telemetry).
