---
title: Langfuse
description: Monitor, evaluate and debug your AI TOOLKIT application with Langfuse
---

# Langfuse Observability

[Langfuse](https://langfuse.com/) ([GitHub](https://github.com/langfuse/langfuse)) is an open source LLM engineering platform that helps teams to collaboratively develop, monitor, and debug AI applications. Langfuse integrates with the AI TOOLKIT to provide:
...
The AI TOOLKIT supports tracing via OpenTelemetry. With the `LangfuseExporter` you can collect these traces in Langfuse.
While telemetry is experimental ([docs](/docs/ai-toolkit-core/telemetry#enabling-telemetry)), you can enable it by setting `experimental_telemetry` on each request that you want to trace.
...
export function register() {
  registerOTel({
    serviceName: 'langfuse-khulnasoft-ai-nextjs-example',
...
Done! All traces that contain AI TOOLKIT spans are automatically captured in Langfuse.

## Example Application

Check out the sample repository ([langfuse/langfuse-khulnasoft-ai-nextjs-example](https://github.com/langfuse/langfuse-khulnasoft-ai-nextjs-example)) based
on the [next-openai](https://github.com/khulnasoft/ai/tree/main/examples/next-openai) template to showcase the integration of Langfuse with Next.js and AI TOOLKIT.

## Configuration

### Group multiple executions in one trace

You can open a Langfuse trace and pass the trace ID to AI TOOLKIT calls to group multiple execution spans under one trace. The passed name in `functionId` will be the root span name of the respective execution.
...
![Khulnasoft nested trace in Langfuse UI](https://langfuse.com/images/docs/vercel-nested-trace.png)
...
You can link Langfuse prompts to AI TOOLKIT generations by setting the `langfusePrompt` property in the `metadata` field:
...
- If you deploy on Khulnasoft, Khulnasoft's OpenTelemetry Collector is only available on Pro and Enterprise Plans ([docs](https://vercel.com/docs/observability/otel-overview)).
...
- After setting up Langfuse Tracing for the AI TOOLKIT, you can utilize any of the other Langfuse [platform features](https://langfuse.com/docs):
...
- For more information, see the [telemetry documentation](/docs/ai-toolkit-core/telemetry) of the AI TOOLKIT.
