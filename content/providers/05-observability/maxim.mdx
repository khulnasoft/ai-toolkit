...
[Maxim AI](https://getmaxim.ai) streamlines AI application development and deployment by applying traditional software best practices to non-deterministic AI workflows. Our evaluation and observability tools help teams maintain quality, reliability, and speed throughout the AI application lifecycle. Maxim integrates with the AI TOOLKIT to provide:

- Automatic Observability – Adds tracing, logging, and metadata to AI TOOLKIT calls with a simple wrapper.
...
# Setting up Maxim with the AI TOOLKIT
...
## Wrap AI TOOLKIT Models with Maxim

```javascript
import { openai } from '@ai-toolkit/openai';
import { wrapMaximAITOOLKITModel } from '@maximai/maxim-js/vercel-ai-toolkit';

const model = wrapMaximAITOOLKITModel(openai('gpt-4'), logger);
```
...
## Working with Different AI TOOLKIT Functions

The wrapped model works seamlessly with all Vercel AI TOOLKIT functions:
...
```javascript
import { MaximVercelProviderMetadata } from '@maximai/maxim-js/vercel-ai-toolkit';
...
![Maxim Demo](/images/maxim_vercel.gif)
...
import { streamText } from 'ai-toolkit';
import { openai } from '@ai-toolkit/openai';
import { wrapMaximAITOOLKITModel, MaximVercelProviderMetadata } from '@maximai/maxim-js/vercel-ai-toolkit';
...
const openaiModel = wrapMaximAITOOLKITModel(openai('gpt-4'), logger);
const anthropicModel = wrapMaximAITOOLKITModel(
  anthropic('claude-3-5-sonnet-20241022'),
  logger,
);
const googleModel = wrapMaximAITOOLKITModel(google('gemini-pro'), logger);
...
- After setting up Maxim tracing for the Vercel AI TOOLKIT, you can explore other Maxim platform capabilities:
...
For further details, checkout Vercel AI TOOLKIT's [Maxim integration documentation](https://www.getmaxim.ai/docs/sdk/typescript/integrations/vercel/vercel).
