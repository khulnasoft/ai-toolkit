...
The `deepeval-ts` package integrates with the AI TOOLKIT's `experimental_telemetry` API to provide tracing, online evaluations, and session analytics.

## Setup

To enable tracing, install `deepeval-ts`, configure your API key, and initialize a tracer using `configureAiToolkitTracing`.
...
Import and call `configureAiToolkitTracing` to create a `tracer`:

```typescript
import { configureAiToolkitTracing } from 'deepeval-ts';

const tracer = configureAiToolkitTracing();
```

## Tracing Your Application

You can now pass the `tracer` object into the `experimental_telemetry` field of any AI TOOLKIT call to get your traces on the Confident AI platform.

Here are some of the examples on how to trace various AI TOOLKIT functions using Confident AI's tracer:
...
You can customize trace grouping and evaluation behavior by passing options to `configureAiToolkitTracing`. This allows you to:
...
const tracer = configureAiToolkitTracing({
  name: 'AI TOOLKIT Confident AI Tracing',
  threadId: 'thread-123',
  userId: 'user-456',
...
If you use Confident AI Prompt Management, you can associate traces with a specific prompt version. Pass a `Prompt` object to `configureAiToolkitTracing` to associate your traces with the prompt version used at runtime.

```typescript
import { generateText } from 'ai';
import { openai } from '@ai-toolkit/openai';
import { configureAiToolkitTracing, Prompt } from 'deepeval-ts';
...
<Note>
  ...
  `configureAiToolkitTracing`. Without pulling first, the prompt version will not be
...
2. Pass your metric collection name in the `configureAiToolkitTracing` options
...
You can find a more comprehensive guide on AI TOOLKIT tracing with `deepeval-ts` in the [Confident AI docs here](https://www.confident-ai.com/docs/integrations/third-party/vercel-ai-toolkit).
...
- [AI TOOLKIT Telemetry documentation](/docs/ai-toolkit-core/telemetry)
