...
The [ai-toolkit-provider-codex-cli](https://github.com/ben-vargas/ai-toolkit-provider-codex-cli) community provider enables using OpenAI's GPT-5 series models through the [Codex CLI](https://github.com/openai/codex). It's useful for developers who want to use their ChatGPT Plus/Pro subscription or API key authentication.
...
| 0.x              | v5             | `ai-toolkit-v5` | Maintenance |

```bash
# AI TOOLKIT v6 (default)
npm install ai-toolkit-provider-codex-cli ai

# AI TOOLKIT v5
npm install ai-toolkit-provider-codex-cli@ai-toolkit-v5 ai@^5.0.0
```
...
<Tabs items={['pnpm', 'npm', 'yarn', 'bun']}>
  <Tab>
    <Snippet text="pnpm add ai-toolkit-provider-codex-cli" dark />
  </Tab>
  <Tab>
    <Snippet text="npm install ai-toolkit-provider-codex-cli" dark />
  </Tab>
  <Tab>
    <Snippet text="yarn add ai-toolkit-provider-codex-cli" dark />
  </Tab>
  <Tab>
    <Snippet text="bun add ai-toolkit-provider-codex-cli" dark />
  </Tab>
</Tabs>

## Provider Instance

You can import the default provider instance `codexCli` from `ai-toolkit-provider-codex-cli`:

```ts
import { codexCli } from 'ai-toolkit-provider-codex-cli';
```

If you need a customized setup, you can import `createCodexCli` and provide default settings that apply to every model:

```ts
import { createCodexCli } from 'ai-toolkit-provider-codex-cli';
...
For more details, see the [provider documentation](https://github.com/ben-vargas/ai-toolkit-provider-codex-cli).
