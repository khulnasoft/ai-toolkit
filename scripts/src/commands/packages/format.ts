...
const NO_MODULE_PACKAGES = ['ai-toolkit'];
...
const overrides: Record<string, unknown> = {
  license: 'MIT',
  author: 'AI Toolkit (https://github.com/ai-toolkit)',
  bugs: 'https://github.com/ai-toolkit/ai-toolkit/issues/new/choose',
  homepage: 'https://github.com/ai-toolkit/ai-toolkit',
};
...
async function formatPackage(filepath: string) {
  ...
  if (!data.name.startsWith('@ai-toolkit/')) {
    as.yellow(`Package ${data.name} is not in the @ai-toolkit/* scope.`);
  }
  ...
  formattedData['repository'] = {
    type: 'git',
    url: 'git+https://github.com/ai-toolkit/ai-toolkit.git',
    directory,
  };
...
async function formatDirectoryPackageFiles(folder: PackageType, root: string) {
  ...
  await Promise.all(
    packages.map(dir =>
      formatPackage(path.resolve(root, folder, dir, 'package.json')),
    ),
  );
}
...
function validatePackageScripts(
  ...
  required.forEach(scriptName => {
    ...
    if (
      ...
      console.log(
        as.red(
          `Python package ${packageName} MUST have ruff and black configured for linting`,
        ),
      );
    }
  });
