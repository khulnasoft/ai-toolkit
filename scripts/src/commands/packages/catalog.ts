...
export const update = new Command('update')
  ...
  .action(async (input: string[]) => {
    ...
    const packageNames = Object.values(workspace.catalogs).flatMap(c =>
      Object.keys(c),
    );
    ...
    await Promise.all(
      names.map(async n => {
        const catalog = Object.keys(workspace.catalogs).find(c =>
          Object.keys(workspace.catalogs[c]).includes(n),
        );
        ...
        if (catalog) {
          ...
          console.log(
            `Updated ${as.bold.blue(n)} to ${as.bold.green(latest.version)}`,
          );
        }
      }),
    );
    ...
    console.log(
      as.green('Packages updated successfully. Please run `pnpm install`.'),
    );
  });
...
type PackageUpdates = Record<
  string,
  { current: string; latest: string; error: string }
>;
...
async function checkVersions(packages: string[]) {
  ...
  const longestVersion =
    Math.max(...Object.values(updates).map(u => u.current.length)) + 2;
  ...
  Object.entries(updates).forEach(([name, update]) => {
    if (
      ...
      console.log(
        as.redBright(`${name} has an invalid version: ${update.latest}`),
      );
      return;
    }

    const strip = (v: string) =>
      /.*([0-9]+\.[0-9]+\.[0-9]+)$/.exec(v)?.[1] ?? v;
