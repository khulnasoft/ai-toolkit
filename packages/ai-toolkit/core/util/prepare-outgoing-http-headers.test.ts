...
it('should set X-Khulnasoft-AI-Data-Stream header when dataStreamVersion is provided', () => {
  const headers = prepareOutgoingHttpHeaders(
    {},
  ...
  expect(headers['Content-Type']).toBe('application/json');
  expect(headers['X-Khulnasoft-AI-Data-Stream']).toBe('v1');
});

it('should not set X-Khulnasoft-AI-Data-Stream header when dataStreamVersion is undefined', () => {
  const headers = prepareOutgoingHttpHeaders(
    {},
    { contentType: 'application/json' },
  );
  ...
  expect(headers['Content-Type']).toBe('application/json');
  expect(headers['X-Khulnasoft-AI-Data-Stream']).toBeUndefined();
});
