...
export const codexACPHarness = createACP({
  ...
  credentialBrokering: ({ env, sandboxEnv }) => {
    ...
    // Match on the exact masked Authorization header. The effective base
    // URL may be customized via CODEX_CONFIG, so no URL restriction is
    // applied here — the masked header value is high-entropy and unique.
    return [
      {
