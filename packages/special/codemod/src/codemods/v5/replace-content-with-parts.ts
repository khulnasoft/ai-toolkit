...
export default createTransformer((fileInfo: FileInfo, api: API, options, context) => {
  ...
  root
    ...
    .forEach(path => {
      ...
      messagesProp.value.elements.forEach((messageElement: any) => {
        ...
        const contentPropertyIndex = messageElement.properties.findIndex((prop: any) => {
          if (j.Property.check(prop) || j.ObjectProperty.check(prop)) {
            const key = prop.key;
            return (
        ...
        const contentProperty = messageElement.properties[contentPropertyIndex] as any;

        // Check if there's already a parts property
        const hasPartsProperty = messageElement.properties.some((prop: any) => {
          if (j.Property.check(prop) || j.ObjectProperty.check(prop)) {
            const key = prop.key;
            return (
        ...
        const partsProperty = j.property('init', j.identifier('parts'), partsArray);
